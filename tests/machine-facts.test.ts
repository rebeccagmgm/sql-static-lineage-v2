...
function fixture(): {
      ...
      records: [
        {
          ...
          metadata_qualified_name: "DEMO.SOURCE",
          ddl_sha256: "ddl-source",
          required_for_star: true,
          table_status: "ONLINE",
          columns: [
            { name: "id", partition: false },
            { name: "dt", partition: true },
          ],
        },
        {
          ...
          table: "target",
          status: "SUCCESS",
          columns: [{ name: "id", partition: false }],
        },
      ],
  ...
  writeFileSync(
    ...
    JSON.stringify({
      schema_version: "test",
      dialect: "databricks",
      schema_evidence: relative(workspace, schema).replace(/\\/g, "/"),
      tasks: [
        {
          task_id: "test-task",
          sql_snapshot: relative(workspace, sql).replace(/\\/g, "/"),
          writes: "demo.target",
        },
      ],
    }),
  ...
  return {
    root,
    profile: relative(workspace, profile).replace(/\\/g, "/"),
    output: relative(workspace, join(root, "machine-facts")).replace(
      /\\/g,
      "/",
    ),
    sql,
...
describe("machine facts contract", () => {
  it("canonicalizes keys while preserving array order and removes volatile schema fields", () => {
    ...
    expect(
      stripVolatile({
        captured_at: "now",
        nested: { source_path: "x", keep: true },
      }),
    ...
    expect(datasetId("source-a", "Demo.Source")).not.toBe(
      datasetId("source-b", "Demo.Source"),
    );
  });
  ...
  it("merges persisted schema evidence deterministically without task-specific rules", () => {
          ...
          records: [
            {
              ...
              ddl: "CREATE TABLE demo.source",
            },
            {
              qualified_name: "demo.extra",
              status: "SUCCESS",
              columns: [{ name: "key" }],
            },
          ],
    ...
    expect(
      merged.records.find(
        (record: { qualified_name?: string }) =>
          record.qualified_name?.toLowerCase() === "demo.source",
      ),
    ).toMatchObject({
      status: "SUCCESS",
      columns: [{ name: "id" }, { name: "dt" }],
  ...
  it("scopes each task bundle to its SQL inputs and declared output", () => {
    ...
    expect(inspectJsonlStore(join(bundle, "schema-refs.jsonl")).status).toBe(
      "GZIP",
    );
    ...
    expect(schemaRefs.map((record) => record.qualified_name)).toEqual([
      "demo.source",
      "demo.target",
    ]);
  });
  ...
  it("creates one current bundle, reuses it, and replaces it when SQL changes", () => {
    ...
    const schemaRefs = readJsonlText(
      join(
        f.root,
        "machine-facts",
        "registry",
      ...
      .split(/\r?\n/)
      .map((line) => JSON.parse(line));
    const sourceRef = schemaRefs.find(
      (record: { qualified_name?: string }) =>
        record.qualified_name === "demo.source",
    );
    ...
    writeFileSync(
      f.sql,
      "INSERT OVERWRITE TABLE demo.target SELECT id + 1 AS id FROM demo.source;\n",
      "utf8",
    );
    ...
    expect(secondManifest.inputs.sql_sha256).not.toBe(
      firstManifest.inputs.sql_sha256,
    );
  ...
  it("binds root SELECT expressions to an explicit INSERT target by target schema order", () => {
    ...
    expect(
      manifest.outputs.some(
        (output: { path?: string }) =>
          output.path === "output-field-bindings.jsonl",
      ),
    ).toBe(true);
  });
  ...
  it("binds the query producer of a WITH-prefixed INSERT", () => {
    ...
    const bindings = readJsonlText(join(bundle, "output-field-bindings.jsonl"))
      ...
      .filter(Boolean)
      .map((line) => JSON.parse(line));
    const expressions = readJsonlText(
      join(bundle, "field-expression-nodes.jsonl"),
    )
      .trim()
      .split(/\r?\n/)
      .filter(Boolean)
    ...
    expect(statements[0]).toMatchObject({
      statement_type: "INSERT_OVERWRITE",
    ...
    expect(bindings[0]).toMatchObject({
      target_dataset: "demo.target",
      target_field: "id",
      expression_id: expressions.find((expression) =>
        expression.relation_id.endsWith(":root.project"),
      )?.expression_id,
    });
  });
  ...
  it("keeps one Write Observation across a CTE, physical SELECT * subquery, and dynamic partition", () => {
    ...
    schema.records = [
      {
        ...
        required_for_star: true,
        columns: [
          { name: "key_col", partition: false },
          { name: "payload", partition: false },
        ],
      },
      {
        qualified_name: "demo.target",
        status: "SUCCESS",
        columns: [
          { name: "out_col", partition: false },
          { name: "busi_date", partition: true },
        ],
      },
    ];
    writeFileSync(schemaPath, JSON.stringify(schema), "utf8");
    ...
    const profile = JSON.parse(
      readFileSync(join(f.root, "profile.json"), "utf8"),
    );
    ...
    const writes = jsonl("dataset-io.jsonl").filter(
      (record) =>
        record.direction === "WRITE" && record.field_producing === true,
    );
    ...
    expect(
      bindings.find((binding) => binding.target_field === "out_col"),
    ).toMatchObject({
    ...
    expect(
      unknowns.some(
        (unknown) => unknown.reason_code === "PLAN_FACT_UNRESOLVED",
      ),
    ).toBe(false);
  });
  ...
  it("resolves a bare INSERT target to the declared physical table identity", () => {
    ...
    schema.records = [
      ...
      {
        qualified_name: "warehouse.demo.target",
        status: "SUCCESS",
        columns: [{ name: "id", partition: false }],
      },
      {
        ...
        status: "SUCCESS",
        columns: [{ name: "legacy_only", partition: false }],
      },
    ];
    writeFileSync(schemaPath, JSON.stringify(schema), "utf8");
    writeFileSync(
      f.sql,
      "INSERT OVERWRITE TABLE target SELECT id FROM demo.source;\n",
      "utf8",
    );
    ...
    const jsonl = (name: string): any[] => readJsonlRecords(join(bundle, name));
    const write = jsonl("dataset-io.jsonl").find(
      (record) =>
        record.direction === "WRITE" && record.field_producing === true,
    );
    ...
    expect(binding).toMatchObject({
      target_dataset: "warehouse.demo.target",
      target_field: "id",
    });
  });
  ...
  it("binds every resolvable multi-column CTAS producer ordinal to the same Write", () => {
    ...
    const schemaPath = join(f.root, "schema.json");
    const schema = JSON.parse(readFileSync(schemaPath, "utf8"));
    const source = schema.records.find(
      (record: { qualified_name?: string }) =>
        record.qualified_name === "demo.source",
    );
    const target = schema.records.find(
      (record: { qualified_name?: string }) =>
        record.qualified_name === "demo.target",
    );
    source.columns = [
      { name: "id", partition: false },
    ...
    const sameWriteBindings = bindings.filter(
      (binding) => binding.statement_id === writeStatementId,
    );
    const sameWriteGaps = unknowns.filter(
      (gap) =>
        gap.statement_id === writeStatementId &&
        gap.gap_domain === "OUTPUT_BINDING",
    );
    ...
    expect([...disposedOrdinals].sort((left, right) => left - right)).toEqual(
      producerOrdinals,
    );
    expect(sameWriteBindings).toHaveLength(2);
    expect(sameWriteBindings.map((binding) => binding.target_field)).toEqual([
      "id",
      "amount",
    ]);
    expect(
      new Set(sameWriteBindings.map((binding) => binding.write_observation_id))
        .size,
    ).toBe(1);
    expect(
      sameWriteBindings.every(
        (binding) => binding.binding_status === "RESOLVED",
      ),
    ).toBe(true);
  ...
  it("records every unprovable CTAS producer ordinal as a same-Write binding gap", () => {
    ...
    writeFileSync(
      f.sql,
      "CREATE TABLE demo.target AS SELECT id, amount FROM demo.source;\n",
      "utf8",
    );
    ...
    const gap = unknowns.find(
      (item) =>
        item.statement_id === "task:test-task:statement:0" &&
        item.gap_domain === "OUTPUT_BINDING",
    );
  ...
  it("never pairs a CTAS Write with a following independent SELECT Statement", () => {
    ...
    const bindings = readJsonlText(join(bundle, "output-field-bindings.jsonl"))
      ...
      .filter(Boolean)
      .map((line) => JSON.parse(line));
    const ctasExpression = fields.find(
      (field) => field.statement_id === "task:test-task:statement:0",
    );
    const independentExpression = fields.find(
      (field) => field.statement_id === "task:test-task:statement:1",
    );

    expect(
      statements
        .filter((statement) => String(statement.raw_sql).trim().length > 0)
        .map((statement) => statement.statement_type),
    ...
    expect(
      bindings.some(
        (binding) =>
          binding.expression_id === independentExpression.expression_id,
      ),
    ).toBe(false);
  });
  ...
  it("does not guess positional output bindings when target schema and SELECT counts differ", () => {
    ...
    const bindingText = readJsonlText(
      join(bundle, "output-field-bindings.jsonl"),
    ).trim();
    const unknowns = readJsonlText(join(bundle, "unknowns.jsonl"))
      .trim()
  ...
  it("honors an explicit INSERT target column list instead of physical schema position", () => {
    ...
    expect(
    ...
    ).toEqual([
      {
        ...
        sourceOrdinal: 0,
        targetOrdinal: 1,
        method: "EXPLICIT_TARGET_COLUMN_LIST",
        schema: "MATCH",
      },
      {
        target: "id",
  ...
  it("keeps dynamic partition output mapping unresolved", () => {
    ...
    expect(
      readJsonlText(join(bundle, "output-field-bindings.jsonl")).trim(),
    ).toBe("");
    const unknowns = readJsonlText(join(bundle, "unknowns.jsonl"))
      .trim()
      .split(/\r?\n/)
  ...
  it("excludes failed status and corrupted current bundles from the index", () => {
    ...
    writeFileSync(
      gzipJsonlPath(statements),
      gzipCanonicalBytes(`${readJsonlText(statements)}corrupt\n`),
    );
  ...
  it("requires a logical source and rejects same-context output drift", () => {
    const f = fixture();
    const profileWithoutSource = JSON.parse(
      readFileSync(join(f.root, "profile.json"), "utf8"),
    );
    const profilePath = join(f.root, "profile-without-source.json");
    writeFileSync(
      profilePath,
      JSON.stringify({ ...profileWithoutSource, logical_source_id: undefined }),
      "utf8",
    );
    expect(() =>
      processProfile(
        relative(workspace, profilePath).replace(/\\/g, "/"),
        f.output,
      ),
    ).toThrow("logical_source_id is required");
    ...
    expect(result.tasks[0]?.failures[0]?.reason_code).toBe(
  ...
  it("isolates malformed status and manifest during index rebuild", () => {
    ...
    writeFileSync(
      join(taskRoot2, "bundle", "manifest.json"),
  ...
  it("rejects duplicate task identities before writing", () => {
    ...
    expect(() =>
      processProfile(
        relative(workspace, duplicatePath).replace(/\\/g, "/"),
        f.output,
        "test-source",
      ),
    ).toThrow("task_id values must be unique");
  });
  ...
  it("preserves plan unknown classes without manufacturing field-binding unknowns", () => {
    ...
    expect(
      unknowns.some(
        (item) =>
          item.outcome_class === "NOT_APPLICABLE" &&
          item.reason_code === "NON_QUERY_OUTPUT_NOT_APPLICABLE",
      ),
    ).toBe(true);
    expect(
      unknowns.some(
        (item) => item.reason_code === "PHYSICAL_FIELD_BINDING_UNRESOLVED",
      ),
    ).toBe(false);
    expect(
      expressions.some(
        (item) => item.input_dependency_status === "NO_PHYSICAL_INPUT",
      ),
    ).toBe(true);
    expect(
      expressions.some((item) => item.input_dependency_status === "UNRESOLVED"),
    ).toBe(true);
    ...
    expect(
      Object.values(
        manifest.counts.unknowns_by_outcome as Record<string, number>,
      ).reduce((sum, value) => sum + value, 0),
    ).toBe(manifest.counts.unknowns);
  });
        ...
        input_columns: [
          {
            name: "known_id",
            resolution: "PHYSICAL",
            physical: [{ table: "demo.source", column: "known_id" }],
          },
  ...
  it("detects missing schema evidence through relation inputs", () => {
    ...
    expect(
      relationNeedsMissingSchema("join", relations, new Set(["demo.source"])),
    ).toBe(true);
    expect(
      relationNeedsMissingSchema(
        "join",
        relations,
        new Set(["demo.source", "demo.missing"]),
      ),
    ).toBe(false);
  });
  ...
  it("keeps qualified missing-schema columns as candidates", () => {
    ...
    writeFileSync(
      f.sql,
      "INSERT OVERWRITE TABLE demo.target SELECT s.id FROM demo.source s JOIN demo.missing m ON s.id = m.id;\n",
      "utf8",
    );
    processProfile(f.profile, f.output, "test-source");
    const bundle = join(
    ...
    expect(
      unknowns.some(
        (item) => item.reason_code === "SCHEMA_BINDING_NOT_EVALUABLE",
      ),
    ).toBe(false);
  });
  ...
  it("does not relabel followColumn parser unknowns as missing schema", () => {
    ...
    writeFileSync(
      f.sql,
      "CREATE TABLE demo.target AS SELECT A.*, B.* FROM (SELECT id FROM demo.source) A FULL OUTER JOIN (SELECT id FROM demo.missing) B ON A.id = B.id;",
      "utf8",
    );
    processProfile(f.profile, f.output, "test-source");
    const bundle = join(
    ...
    expect(
      unknowns.filter(
        (item) =>
          ...
          item.message.includes("followColumn 无来源"),
      ),
    ).toHaveLength(0);
    expect(
      unknowns.filter(
        (item) =>
          item.reason_code === "PHYSICAL_FIELD_UNRESOLVED" &&
          item.message.includes("followColumn 无来源"),
      ),
    ).toHaveLength(0);
    expect(
      unknowns.some(
        (item) =>
          item.reason_code === "SCHEMA_BINDING_NOT_EVALUABLE" &&
          item.message.includes("demo.missing"),
      ),
    ).toBe(true);
  });
  ...
  it("passes nested schema namespaces to derived-table star resolution", () => {
    ...
    writeFileSync(
      f.sql,
      "CREATE TABLE demo.target AS SELECT A.id FROM (SELECT id FROM demo.source) A;",
      "utf8",
    );
    processProfile(f.profile, f.output, "test-source");
    const bundle = join(
    ...
    expect(
      unknowns.filter(
        (item) => item.reason_code === "PHYSICAL_FIELD_UNRESOLVED",
      ),
    ).toHaveLength(0);
    expect(
      unknowns.filter((item) => item.reason_code === "PLAN_FACT_UNRESOLVED"),
    ).toHaveLength(0);
  });
  ...
  it("records syntax-only candidates when a single source schema is absent", () => {
    ...
    expect(
      expressions.some(
        (item) => item.input_dependency_status === "SQL_CANDIDATE",
      ),
    ).toBe(true);
    expect(
      expressions.some((item) =>
        item.candidate_input_fields?.some(
          (field: { binding_status?: string }) =>
            field.binding_status === "UNVERIFIED_SCHEMA",
        ),
      ),
    ).toBe(true);
    expect(
      lineage.some(
        (item) =>
          item.resolution_status === "UNVERIFIED_SCHEMA" &&
          item.method === "SQL_SINGLE_SOURCE_BINDING",
      ),
    ).toBe(true);
    const unknowns = readJsonlText(join(bundle, "unknowns.jsonl"))
      .trim()
  ...
  it("keeps missing-schema facts unevaluable for star expansion", () => {
    ...
    writeFileSync(
      f.sql,
      "INSERT OVERWRITE TABLE demo.target SELECT * FROM demo.source;\n",
      "utf8",
    );
    ...
    expect(
      unknowns.some(
        (item) =>
          item.outcome_class === "NOT_EVALUABLE" &&
          item.reason_code === "SCHEMA_BINDING_NOT_EVALUABLE",
      ),
    ).toBe(true);
  });
  ...
  it("treats scheduler date placeholders as reversible parser values", () => {
    ...
    const nonEmptyStatements = statements.filter(
      (item) => String(item.raw_sql).trim().length > 0,
    );
    expect(nonEmptyStatements.length).toBeGreaterThan(0);
    expect(
      nonEmptyStatements.every((item) => item.parse_status === "SUCCESS"),
    ).toBe(true);
    expect(statements.map((item) => item.raw_sql).join("\n")).toContain(
      "${yyyyMM,-1M}",
    );
    expect(
      relations.some((item) => item.relation?.table === "demo.trd_${yyyyMM}_h"),
    ).toBe(true);
    expect(
      relations.some(
        (item) => item.relation?.table === "demo.trd_${yyyyMM,-1M}_h",
      ),
    ).toBe(true);
    expect(
      relations.some((item) => item.relation?.binding === "trd_${yyyyMM}_h"),
    ).toBe(true);
    expect(
      relations.some(
        (item) => item.relation?.binding === "trd_${yyyyMM,-1M}_h",
      ),
    ).toBe(true);
    expect(
      unknowns.some((item) => item.reason_code === "SYNTAX_DIAGNOSTIC"),
    ).toBe(false);
  });
  ...
  it("carries Plan Facts raw/display, roles, clauses, occurrences, window frame, and Top-N into Machine Facts", () => {
    ...
    const reads = jsonl("dataset-io.jsonl").filter(
      (record) => record.direction === "READ",
    );
    const unknowns = jsonl("unknowns.jsonl");
    const coalesce = expressions.find((record) =>
      record.expression_text.includes("COALESCE"),
    );
    ...
    expect(window.window_spec.frame).toMatchObject({
      status: "UNKNOWN",
      source_span: expect.any(Object),
    });
    expect(
      relations
        .filter((record) => record.relation_type === "filter")
        .map((record) => record.relation.clause),
    ).toEqual(["where", "where", "having", "qualify"]);
    expect(
      relations.some(
        (record) =>
          record.relation_type === "top_n" &&
          record.relation.limit.fetch === undefined &&
          record.relation.limit.kind === "LIMIT",
      ),
    ).toBe(true);
    expect(reads[0]?.read_occurrences).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          occurrence_id: expect.stringContaining("relation"),
          relation_id: expect.stringContaining("relation"),
          source_span: expect.any(Object),
        }),
      ]),
    );
    expect(
      unknowns.some(
        (record) =>
          record.reason_code === "PLAN_FACT_UNRESOLVED" &&
          record.source_locator?.start !== undefined,
      ),
    ).toBe(true);
  });
});
