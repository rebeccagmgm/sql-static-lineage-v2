...
const question = (
  taskId: string,
  outputFieldSeed: readonly string[] = ["target_id"],
): QuestionSpec => ({
  ...
  stop_condition:
    "停止于静态证据闭合或明确记录缺失能力，不推断运行值或业务口径。",
});
...
function refreshAttestation(factsRoot: string, taskId: string): void {
  ...
  manifest.outputs = manifest.outputs.map((output) => ({
    ...
    ...(String(output.path).endsWith(".jsonl")
      ? {
          row_count: readFileSync(
            join(bundle, String(output.path)),
            "utf8",
          ).trim()
  ...
  writeJsonl(
    indexPath,
    rows.map((row) =>
      row.task_id === taskId ? { ...row, manifest_sha256: manifestHash } : row,
    ),
  );
}

function createL1Facts(options: {
  kind: "insert" | "ctas" | "unprovable";
  independentSelect?: boolean;
}): {
  root: string;
  taskId: string;
} {
  const root = mkdtempSync(
    join("C:\\Users\\13246\\AppData\\Local\\Temp", "task-inspection-canary-"),
  );
  roots.push(root);
  ...
  const sql = options.independentSelect
    ? `${primarySql}\nSELECT id FROM demo.other;`
    : primarySql;
  ...
  writeFileSync(
    join(factsRoot, "snapshots", "sql", `${sqlHash}.sql`),
    sql,
    "utf8",
  );
  writeFileSync(
    join(factsRoot, "snapshots", "schema", `${schemaHash}.json`),
    schema,
    "utf8",
  );
  const statement = {
  ...
  const expression = {
    ...
    input_dependency_status:
      options.kind === "unprovable" ? "UNRESOLVED" : "PHYSICAL",
    input_fields:
        ...
        : [
            {
              ...
              binding_method:
                options.kind === "ctas"
                  ? "SQL_CREATE_POSITIONAL"
                  : "EXPLICIT_TARGET_COLUMN_LIST",
  ...
  for (const [file, records] of Object.entries(files))
    writeJsonl(join(bundle, file), records);
...
describe("Task Inspection Consumer", () => {
  it("renders a resolved INSERT with full formula, source and evidence references", () => {
    ...
    expect(card.output_fields[0].source_fields[0].field_id).toBe(
      "field:test:demo.source.id",
    );
    expect(
      card.output_fields[0].evidence_refs.some((ref: string) =>
        ref.includes("output-field-bindings.jsonl#L1"),
      ),
    ).toBe(true);
  ...
  it("keeps resolvable CTAS bound to its same Write and does not pair an independent SELECT", () => {
    ...
    expect(card.output_fields[0].binding.query_producer_statement_id).toBe(
      "task:canary-ctas:statement:0",
    );
  ...
  it("fails closed for an unprovable CTAS/Star without inventing output binding", () => {
    ...
    expect(card.gaps.grouped.OUTPUT_BINDING[0].reason_code).toBe(
  ...
  it("returns STALE and no stale facts when the Current Index no longer attests the Manifest", () => {
    ...
    expect(card.next_verification.evidence_type).toBe(
      "CURRENT_INDEX_AND_MANIFEST_HASH",
    );
  });
  ...
  it("fails closed for duplicate Index rows and an L1 output omitted from Manifest", () => {
    ...
    expect(loadCurrentTaskBundle(duplicate.root, duplicate.taskId).state).toBe(
      "INVALID",
    );
    ...
    manifest.outputs = manifest.outputs.filter(
      (output: { path: string }) => output.path !== "statements.jsonl",
    );
    writeJson(manifestPath, manifest);
    ...
    expect(load.issues).toContain(
      "L1_REQUIRED_OUTPUT_NOT_DECLARED:statements.jsonl",
    );
  });
  ...
  it("rejects a binding whose producer statement is not the Write producer", () => {
    ...
    const bindings = readFileSync(bindingPath, "utf8")
      ...
      .map((line) => ({
        ...JSON.parse(line),
        query_producer_statement_id: `task:${facts.taskId}:statement:1`,
      }));
    ...
    expect(card.gaps.grouped.OUTPUT_BINDING[0].reason_code).toBe(
      "OUTPUT_BINDING_IDENTITY_NOT_CLOSED",
    );
  });
  ...
  it("downgrades a resolved field when a blocking gap covers the same task", () => {
    ...
    expect(card.gaps.grouped.OUTPUT_BINDING[0].reason_code).toBe(
      "TARGET_SCHEMA_NOT_EVALUABLE",
    );
    expect(card.gaps.grouped.OUTPUT_BINDING[0].evidence_refs[0]).toContain(
      "unknowns.jsonl#L2",
    );
  });
  ...
  it("rejects unsupported contracts, missing task scope and untraceable spans", () => {
    ...
    const unsupportedManifest = JSON.parse(
      readFileSync(unsupportedManifestPath, "utf8"),
    );
    ...
    expect(
      loadCurrentTaskBundle(unsupported.root, unsupported.taskId).issues,
    ).toContain("UNSUPPORTED_CONTRACT_VERSION:2.1.0");
    ...
    expect(
      loadCurrentTaskBundle(missingTaskId.root, missingTaskId.taskId).issues,
    ).toContain("OUTPUT_TASK_ID_MISSING:statements.jsonl");
  ...
  it("changes reader identity when the Question Spec changes and keeps the static boundary visible", () => {
    ...
    const second = inspectTask({
      ...
      taskId: facts.taskId,
      questionSpec: {
        ...question(facts.taskId, ["other_id"]),
        stop_condition: "stop",
      },
    });
  ...
  it("escapes task, expression and gap content in the static page", () => {
    ...
    const html = renderTaskInspectionHtml({
      ...
      output_fields: [
        {
          ...card.output_fields[0],
          expression: `</pre><script>alert(2)</script>`,
        },
      ],
      gaps: {
        count: 1,
        grouped: {
          OUTPUT_BINDING: [{ message: `<img src=x onerror=alert(3)>` }],
        },
      },
    });
