...
} from "../../scripts/reconcile/consumer/target-table-upstream-causal-closure/task-relation-summary.ts";
...
function load(
  ...
  return {
    ...
    statusPath: "",
    records: {
      "output-field-bindings.jsonl": writeObservationIds.map(
        (writeObservationId) => ({
          task_id: taskId,
...
describe("closure-on-union C0/C1", () => {
  it("parses the new CLI flags and defaults to legacy", () => {
    ...
    expect(() =>
      parseArgs([...common, "--candidate-source", "union-v2"]),
    ).toThrow("ARGUMENT_MISSING:continuation-index");
    expect(
      parseArgs([
        ...common,
        "--candidate-source",
  ...
  it("attaches exact scopes, prunes DISJOINT, counts one ambiguous read, and removes schedule-only branches", () => {
    ...
    const continuation = createUnionContinuationCandidateSource(
      index([
        entry(readId, [
          candidate(),
          candidate({
          ...
          candidate({
            writeObservationId: "write-observation:105387:9",
            targetWriteNodeId: "target-write:105387:9",
    ...
    expect(
      result.universe.branches.some(
        (value) => value.branchKind === "SCHEDULE_ONLY",
      ),
    ).toBe(false);
    const physical = result.universe.branches.filter(
      (value) => value.branchKind === "PHYSICAL_PRODUCER",
    );
    ...
    expect(
      physical.every(
        (value) =>
          value.writeScope?.rootRelationId ===
          "task:105387:statement:0:relation:root.write",
      ),
    ).toBe(true);
    expect(
      physical.find((value) => value.writeObservationId?.endsWith(":3"))
        ?.continuation?.l1Eligible,
    ).toBe(true);
    expect(
      physical.find((value) => value.writeObservationId?.endsWith(":9"))
        ?.continuation?.partitionMatchStatus,
    ).toBe("ASSUMED");
    expect(result.stats).toEqual({ resolved: 1, ambiguous: 1, missing: 0 });
    expect(result.continuationStats).toEqual({
      l1: 1,
      l2Assumed: 1,
      l2Unknown: 0,
      piOnly: 0,
  ...
  it("counts an L1 candidate but does not resolve it when exact scope binding is missing", () => {
    ...
    expect(
      result.universe.branches[0]?.gapRefs.some((gap) =>
        gap.includes("PRODUCER_WRITE_SCOPE_UNRESOLVED"),
      ),
    ).toBe(true);
    expect(result.stats.resolved).toBe(0);
    expect(result.continuationStats).toMatchObject({
      l1: 1,
      l2Assumed: 0,
      l2Unknown: 0,
      piOnly: 0,
    ...
    const continuation = createUnionContinuationCandidateSource(
      index([
        entry(readC, [
          candidate({
            taskId: "SRC_TBL",
            writeObservationId: "write-observation:SRC_TBL:0",
            partitionMatchStatus: "DISJOINT",
            evidenceLayer: "L2",
            l1Eligible: false,
          }),
        ...
        entry(readK, [
          candidate({
            taskId: "SRC_TBL",
            writeObservationId: "write-observation:SRC_TBL:1",
            partitionMatchStatus: "DISJOINT",
            evidenceLayer: "L2",
            l1Eligible: false,
          }),
  ...
  it("reuses the schedule selector during enrichment and keeps a rejected bridge UNKNOWN", () => {
    ...
    const result = enrichUnionV2ProducerBridges(
      ...
      createUnionContinuationCandidateSource(
        index([entry(readId, [candidate({ taskId: "105387" })])]),
      ),
      noopLoad,
    ...
    expect(result.universe.branches[0]?.boundaryReason).toBe(
  ...
  it("preserves 105387 #3/#6 alignment ambiguity and never shares PI :0", () => {
      ...
      index([
        entry(readId, [
          candidate({
            ...
            evidenceLayer: "L2",
            l1Eligible: false,
            alignmentGapCode: "WRITE_OBSERVATION_ALIGNMENT_AMBIGUOUS",
    ...
    expect(
      physical.every(
        (value) => value.continuation?.partitionMatchStatus === "UNKNOWN",
      ),
    ).toBe(true);
    expect(
      physical.every((value) =>
        value.gapRefs.some((gap) =>
          gap.includes("WRITE_OBSERVATION_ALIGNMENT_AMBIGUOUS"),
        ),
      ),
    ).toBe(true);
    expect(
      physical.some((value) => value.writeObservationId?.endsWith(":0")),
    ).toBe(false);
  ...
  it("marks an exact indexed read as unmatched instead of falling back to table writes", () => {
    ...
    expect(result.universe.branches).toHaveLength(1);
    expect(result.universe.branches[0]?.writeObservationId).toBe(
      preScoped.writeObservationId,
    );
    expect(result.universe.branches[0]?.writeScope).toEqual(
      preScoped.writeScope,
    );
    expect(result.universe.branches[0]?.gapRefs).toContain(
      "continuation-gap:branch:read-not-indexed:CONTINUATION_READ_NOT_FOUND",
    );
      ...
      readImpacts: [
        {
          ...
          impactChannels: ["ROW_MEMBERSHIP"],
          evidenceRefs: ["synthetic-read"],
          gaps: [],
        },
      ],
      relationCount: 1,
    ...
    const closure = buildCausalClosure({
      ...
      summaries: new Map([
        [
          relationSummaryKey(
            "119044",
            "task:119044:slot:query",
            0,
            "task:119044:statement:0:relation:root",
          ),
          summary,
        ],
      ]),
      ...
      rootWriteScope: root.writeScope
        ? {
            taskId: "119044",
            writeObservationId: root.writeObservationId!,
            ...root.writeScope,
          }
        : undefined,
    });
    const assessment = closure.assessments.find(
      (value) => value.candidateBranchId !== root.candidateBranchId,
    );
    expect(
      assessment?.channelAssessments.find(
        (value) => value.channel === "ROW_MEMBERSHIP",
      )?.status,
    ).toBe("CONDITIONAL");
  });
});
