...
import { machineFactsFieldId as fieldId } from "../../src/contracts/identity.js";
...
export const TASK_INSPECTION_SCHEMA_VERSION =
  "machine-facts-task-inspection-v1";
...
function recordAt(
  ...
  return {
    ...
    ref: `${load.evidence[file] ?? `machine-facts:${file}`}#L${index + 1}`,
  };
}
...
function evidenceForField(
  ...
  const found = recordAt(
    ...
    file,
    (candidate) =>
      candidate.expression_id === record.expression_id ||
      candidate.binding_id === record.binding_id,
  );
...
function physicalInputFields(expression: JsonRecord | undefined): JsonRecord[] {
  ...
  if (Array.isArray(expression.input_fields))
    return expression.input_fields.map((field) => ({
      logical_source_id:
        expression.logical_source_id ?? field.logical_source_id,
      field_id: field.field_id,
      table: field.table,
      column: field.column,
    }));
  const columns = Array.isArray(expression.input_columns)
    ? expression.input_columns
    : [];
  return columns.flatMap((column: JsonRecord) =>
    (Array.isArray(column.physical) ? column.physical : []).map(
      (field: JsonRecord) => ({
        logical_source_id:
          expression.logical_source_id ?? field.logical_source_id,
        field_id: field.field_id,
...
function bindingValidationIssues(
  ...
  const write = writes.find(
    (record) => record.write_observation_id === binding.write_observation_id,
  );
  if (!write) issues.push("BINDING_WRITE_NOT_FOUND");
  if (
    !binding.write_observation_id ||
  ...
  if (!binding.target_field_id || !binding.target_dataset_id)
    issues.push("BINDING_TARGET_IDENTITY_INCOMPLETE");
  if (!binding.target_field) issues.push("BINDING_TARGET_FIELD_MISSING");
  if (duplicateTargetBindingIds.has(String(binding.binding_id)))
    issues.push("BINDING_TARGET_ORDINAL_DUPLICATE");
  if (
    logicalSourceId &&
    typeof binding.target_dataset === "string" &&
    typeof binding.target_field === "string" &&
    binding.target_field_id !==
      fieldId(logicalSourceId, binding.target_dataset, binding.target_field)
  )
  ...
  const writeStatement = statements.find(
    (record) => record.statement_id === binding.write_statement_id,
  );
  const producerStatement = statements.find(
    (record) => record.statement_id === binding.query_producer_statement_id,
  );
  if (!writeStatement || !producerStatement)
    issues.push("BINDING_STATEMENT_NOT_FOUND");
  if (expression) {
    if (expression.statement_id !== binding.query_producer_statement_id)
      issues.push("BINDING_PRODUCER_EXPRESSION_MISMATCH");
    if (expression.ordinal !== binding.source_ordinal)
      issues.push("BINDING_SOURCE_ORDINAL_MISMATCH");
    ...
    for (const fieldId of sourceFieldIds)
      if (
        !lineageEdges.some(
          (edge) =>
            edge.to_expression_id === expression.expression_id &&
            edge.from_field_id === fieldId,
        )
      )
  ...
  if (
    !lineageEdges.some(
      (edge) => edge.to_expression_id === binding.expression_id,
    )
  )
...
function gapDomain(record: JsonRecord): string {
  ...
  if (
    ...
    reason.includes("TARGET_SCHEMA") ||
    reason.includes("OUTPUT")
  )
    return "OUTPUT_BINDING";
  if (
    reason.includes("STAR") ||
    reason.includes("ORIGIN") ||
    reason.includes("PHYSICAL_FIELD")
  )
    return "BASE_ORIGIN";
  if (
...
function gapRecord(
  ...
  return {
    ...
    ...(typeof record.artifact_id === "string"
      ? { evidence_ids: [record.artifact_id] }
      : {}),
  };
}
...
function capability(
  ...
  const summaryMismatch =
    rawSubjectCount !== undefined && Number(rawSubjectCount) !== subjectCount;
  return {
    capability: name,
    ...
    ...(Array.isArray(raw?.subject_ids)
      ? { subject_ids: [...raw.subject_ids].sort() }
      : {}),
  };
}
...
function capabilities(
  ...
  return [
    ...
    capability(
      "BASE_ORIGIN",
      expressions.filter(
        (record) => record.input_dependency_status !== "NO_PHYSICAL_INPUT",
      ).length,
      load,
      readRaw("BASE_ORIGIN"),
    ),
...
function answerStatus(
  ...
  const blocking =
    ...
    gaps.some((gap) =>
      ["OUTPUT_BINDING", "BASE_ORIGIN", "ROWSET_CONTROL", "FRESHNESS"].includes(
        String(gap.gap_domain),
      ),
    );
  const partial =
    capabilitiesValue.some(
      (item) =>
        item.coverage === "PARTIAL" || item.coverage === "PROFILE_ASSISTED",
    ) ||
  ...
  if (
    ...
    capabilitiesValue.some(
      (item) =>
        item.capability === "OUTPUT_BINDING" &&
        item.coverage === "NOT_EVALUABLE",
    )
  )
    return "NOT_EVALUABLE";
  if (blocking && resolved) return "PARTIAL";
  if (partial && resolved) return "PARTIAL";
  if (blocking || outputFields.length === 0 || rowsetControls.length === 0)
    return "NOT_EVALUABLE";
...
function nextVerification(
  ...
  if (load.state === "LEGACY_NOT_L1") {
    return {
      ...
      artifact_paths: [
        load.indexPath,
        `${load.bundleDir}\\manifest.json`,
        `${load.bundleDir}\\statements.jsonl`,
      ],
  ...
  return {
    action: firstGap
      ? `核验 ${firstGap.reason_code} 的本地证据`
      : "无需继续静态推断；转入运行/业务核验",
    artifact_paths: firstGap?.evidence_refs ?? [
      load.evidence["manifest.json"] ?? "machine-facts:manifest.json",
    ],
    evidence_type: firstGap?.gap_domain ?? "STATIC_BUNDLE_COMPLETENESS",
    confirmation_role: firstGap
      ? "SQL/Schema evidence reviewer"
      : "Scheduler or business owner",
    stop_condition: question.stop_condition,
  };
}

export function inspectTask(options: TaskInspectionOptions): JsonRecord {
  if (options.questionSpec.task_id !== options.taskId)
    throw new Error("QUESTION_TASK_ID_MISMATCH");
  if (
  ...
  const expressionById = new Map(
    expressions.map((record) => [record.expression_id, record]),
  );
  ...
  const bindingFields = bindings
    ...
    .filter((binding) => {
      ...
      return (
        seeds.length === 0 ||
        seeds.some(
          (seed) => normalize(seed) === normalize(binding.target_field),
        )
      );
    })
    .map((binding) => {
      ...
      return {
        ...
        binding: {
          ...
          query_producer_statement_id:
            binding.query_producer_statement_id ?? null,
        ...
        evidence_ids: Array.isArray(binding.evidence_refs)
          ? [...binding.evidence_refs].sort(compareText)
          : [],
      };
    });
    ...
    ...unknowns
      ...
      .sort(
        (left, right) =>
          ...
          compareText(
            stableRecordKey(left.record),
            stableRecordKey(right.record),
          ),
      )
  ...
  const groupedGaps = Object.fromEntries(
    [...new Set(gaps.map((gap) => gap.gap_domain))]
      ...
      .map((domain) => [
        domain,
        gaps.filter((gap) => gap.gap_domain === domain),
      ]),
  );
  ...
  const card: JsonRecord = {
    ...
    task: {
      ...
      logical_source_id:
        load.manifest?.logical_source_id ??
        load.indexRow?.logical_source_id ??
        null,
      ...
      sql_sha256:
        load.manifest?.inputs?.sql_sha256 ?? load.indexRow?.sql_sha256 ?? null,
    ...
    answer: {
      ...
      confirmation_role:
        load.state === "CURRENT_L1"
          ? "Static evidence reviewer"
          : "Machine Facts method owner",
      unknown:
        load.state === "CURRENT_L1"
          ? null
          : "当前 Bundle 不足以形成 L1 当前证据结论",
    ...
    io: {
      reads: datasetIo
        ...
        .sort(
          (left, right) =>
            normalize(left.physical_dataset).localeCompare(
              normalize(right.physical_dataset),
            ) ||
            compareText(left.statement_id, right.statement_id) ||
      ...
      writes: datasetIo
        ...
        .sort(
          (left, right) =>
            ...
            compareText(
              left.write_observation_id,
              right.write_observation_id,
            ) ||
            compareText(left.statement_id, right.statement_id) ||
            compareText(left.provenance, right.provenance) ||
            compareText(stableRecordKey(left), stableRecordKey(right)),
    ...
    rowset_controls: {
      status:
        load.state === "CURRENT_L1" && rowsetControls.length > 0
          ? "READY"
          : "NOT_EVALUABLE",
    ...
    evidence: {
      manifest_ref:
        load.evidence["manifest.json"] ??
        `${load.indexPath}#task=${options.taskId}`,
      statement_refs: statements
        .slice()
        .sort(
          (left, right) =>
            compareText(left.statement_id, right.statement_id) ||
            Number(left.statement_index ?? 0) -
              Number(right.statement_index ?? 0) ||
            Number(spanOf(left)?.start ?? 0) -
              Number(spanOf(right)?.start ?? 0) ||
            compareText(stableRecordKey(left), stableRecordKey(right)),
        )
        .map((record) => ({
          statement_id: record.statement_id,
      ...
      bundle_files: Object.fromEntries(
        Object.entries(load.evidence).sort(([left], [right]) =>
  ...
  card.result_identity_sha256 = sha256(
    canonicalJson({ ...card, result_identity_sha256: null }),
  );
  return card;
}
...
export function renderTaskInspectionHtml(card: JsonRecord): string {
  ...
  const controls = Array.isArray(card.rowset_controls?.records)
    ? card.rowset_controls.records
    : [];
  const capabilitiesValue = Array.isArray(card.capabilities)
    ? card.capabilities
    : [];
...
export function writeTaskInspection(
  options: TaskInspectionOptions & { readonly outputDir: string },
): JsonRecord {
  ...
  for (const file of [
    resolve(outputDir, "task-inspection.json"),
    resolve(outputDir, "index.html"),
  ])
    if (lstatSafe(file)?.isSymbolicLink())
      throw new Error("DERIVED_OUTPUT_SYMLINK_FILE");
...
function main(): void {
  ...
  const questionSpec = JSON.parse(
    readFileSync(resolve(questionPath), "utf8"),
  ) as QuestionSpec;
  const card = writeTaskInspection({
    factsRoot,
    taskId,
    questionSpec,
    outputDir,
