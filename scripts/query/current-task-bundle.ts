...
export type CurrentBundleState =
  "CURRENT_L1" | "LEGACY_NOT_L1" | "STALE" | "INVALID";
...
function bundleOutputExists(bundleDir: string, file: string): boolean {
  return file.endsWith(".jsonl")
    ? bundleJsonlExists(bundleDir, file)
    : existsSync(join(bundleDir, file));
}
...
function realPathContained(root: string, target: string): boolean {
  try {
    ...
    return (
      escaped === "" || (!escaped.startsWith("..") && !isAbsolute(escaped))
    );
  } catch {
    return false;
  }
...
function validSpan(
  ...
  return (
    ...
    (!statementSpan ||
      (start >= statementSpan.start && end <= statementSpan.end))
  );
}
...
function readIndexSnapshot(indexPath: string): IndexSnapshot {
  try {
    ...
    return {
      rows: null,
      error: `CURRENT_INDEX_INVALID:${error instanceof Error ? error.message : String(error)}`,
    };
  }
}
...
function issueResult(
  ...
  return {
    ...
    bundleDir:
      partial.bundleDir ??
      join(factsRoot, "registry", "tasks", taskId, "bundle"),
    indexPath,
    indexRow: partial.indexRow,
    indexSha256: partial.indexSha256,
    statusPath:
      partial.statusPath ??
      join(factsRoot, "registry", "tasks", taskId, "analysis-status.json"),
...
function loadCurrentTaskBundleWithContext(
  ...
  if (!safeTaskId(taskId))
    return issueResult(factsRoot, taskId, indexPath, ["UNSAFE_TASK_ID"]);
  if (!existsSync(indexPath))
    return issueResult(factsRoot, taskId, indexPath, ["CURRENT_INDEX_MISSING"]);
  ...
  if (snapshot.rows === null)
    return issueResult(factsRoot, taskId, indexPath, [
      snapshot.error ?? "CURRENT_INDEX_INVALID",
    ]);
  ...
  if (indexRow.status !== "SUCCESS")
    return issueResult(
      ...
      taskId,
      indexPath,
      ["INDEX_ROW_NOT_SUCCESS"],
      "STALE",
      { indexRow, indexSha256 },
    );
  ...
  if (!bundleDir || resolve(bundleDir) !== resolve(expectedBundle)) {
    return issueResult(
      ...
      taskId,
      indexPath,
      ["INDEX_BUNDLE_PATH_UNSAFE_OR_UNEXPECTED"],
      "INVALID",
      {
        indexRow,
        indexSha256,
  ...
  if (
    indexRow.manifest_sha256 !== manifestSha256 ||
    status.current_manifest_sha256 !== manifestSha256
  )
  ...
  if (
    status.requested?.schema_bundle_sha256 !==
  ...
  if (status.requested?.dialect !== manifest.method?.dialect)
    staleIssues.push("DIALECT_MISMATCH");
  ...
  if (!isL1 && !isLegacy)
    integrityIssues.push(
      `UNSUPPORTED_CONTRACT_VERSION:${contractVersion || "MISSING"}`,
    );
  ...
  const sqlSnapshotPath = safeRelativePath(
    factsRoot,
    manifest.inputs?.sql_snapshot,
  );
  const schemaSnapshotPath = safeRelativePath(
    factsRoot,
    manifest.inputs?.schema_snapshot,
  );
  if (
    !sqlSnapshotPath ||
    !existsSync(sqlSnapshotPath) ||
    !realPathContained(factsRoot, sqlSnapshotPath)
  )
    integrityIssues.push("SQL_SNAPSHOT_MISSING_OR_UNSAFE");
  else if (
    sha256(readFileSync(sqlSnapshotPath)) !== manifest.inputs?.sql_sha256
  )
    integrityIssues.push("SQL_SNAPSHOT_HASH_MISMATCH");
  if (
    !schemaSnapshotPath ||
    !existsSync(schemaSnapshotPath) ||
    !realPathContained(factsRoot, schemaSnapshotPath)
  )
    integrityIssues.push("SCHEMA_SNAPSHOT_MISSING_OR_UNSAFE");
  else if (
    sha256(readFileSync(schemaSnapshotPath)) !==
    manifest.inputs?.schema_bundle_sha256
  )
  ...
  if (requestedFiles) {
    for (const file of requestedFiles) {
      if (!outputRecords.some((output) => output && output.path === file))
        integrityIssues.push(`REQUESTED_OUTPUT_NOT_DECLARED:${file}`);
      if (!bundleOutputExists(bundleDir, file))
        integrityIssues.push(`REQUESTED_OUTPUT_MISSING:${file}`);
    }
  }
  ...
  for (const output of outputRecords) {
    if (
      ...
      integrityIssues.push(
        `OUTPUT_DESCRIPTOR_INVALID_OR_DUPLICATE:${String(output?.path)}`,
      );
      continue;
    }
    outputPaths.add(output.path);
    if (
      !isSha256(output.content_sha256) ||
  ...
  if (isL1)
    for (const required of REQUIRED_L1_FILES) {
      if (!outputPaths.has(required))
        integrityIssues.push(`L1_REQUIRED_OUTPUT_NOT_DECLARED:${required}`);
      if (!bundleOutputExists(bundleDir, required))
        integrityIssues.push(`L1_REQUIRED_OUTPUT_MISSING:${required}`);
    }
  for (const output of outputRecords) {
    ...
    if (fileName.endsWith(".jsonl")) {
      ...
      if (
        ...
        !realPathContained(bundleDir, stored.path)
      ) {
        integrityIssues.push(`OUTPUT_MISSING:${fileName}`);
        continue;
      }
      if (
        context?.validateOutputHashes !== "requested" ||
        requestedFiles?.has(fileName)
      ) {
  ...
  if (
    !existsSync(sourceArtifactPath) ||
    !realPathContained(bundleDir, sourceArtifactPath)
  )
    ...
    try {
      ...
      integrityIssues.push(
        `SOURCE_ARTIFACT_INVALID:${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  for (const output of outputRecords) {
    ...
    try {
      ...
      if (
        Number.isInteger(output.row_count) &&
        rows.length !== output.row_count
      )
        integrityIssues.push(
          `OUTPUT_ROW_COUNT_MISMATCH:${String(output.path)}`,
        );
      for (const row of rows) {
        if (
          TASK_SCOPED_OUTPUTS.has(String(output.path)) &&
          typeof row.task_id !== "string"
        )
          integrityIssues.push(`OUTPUT_TASK_ID_MISSING:${String(output.path)}`);
        else if (typeof row.task_id === "string" && row.task_id !== taskId)
          integrityIssues.push(
            `OUTPUT_TASK_ID_MISMATCH:${String(output.path)}`,
          );
      }
    } catch (error) {
      integrityIssues.push(
  ...
  if (sqlSnapshotPath && existsSync(sqlSnapshotPath)) {
    ...
    const statementSpans = new Map(
      (parsedJsonl.get("statements.jsonl") ?? []).map((statement) => [
    ...
    for (const statement of parsedJsonl.get("statements.jsonl") ?? []) {
      ...
      if (
        ...
        integrityIssues.push(
          `STATEMENT_SPAN_UNTRACEABLE:${String(statement.statement_id)}`,
        );
    }
      ...
      for (const record of parsedJsonl.get(file) ?? [])
        if (
          !validSpan(
            record[spanField],
            sqlText.length,
            statementSpans.get(record.statement_id),
          )
        )
          integrityIssues.push(
  ...
  const files = requestedFiles
    ...
    : isL1
      ? [
          ...new Set([
            ...REQUIRED_L1_FILES.filter(
              (file) =>
                file !== "capability-summary.json" ||
                existsSync(join(bundleDir, file)),
            ),
            ...declaredFiles,
          ]),
...
export function createCurrentTaskBundleReader(
  ...
  const requestedFiles = options.requestedFiles
    ? new Set(options.requestedFiles)
    : undefined;
  const context: CurrentBundleReadContext = {
    loads: new Map(),
    requestedFiles,
    validateOutputHashes:
      options.validateOutputHashes ?? (requestedFiles ? "requested" : "all"),
  };
  return {
    load: (taskId: string): CurrentBundleLoad => {
      const cached = context.loads.get(taskId);
      if (cached) return cached;
      const loaded = loadCurrentTaskBundleWithContext(
        factsRoot,
        taskId,
        context,
      );
...
export function loadCurrentTaskBundle(
  factsRootInput: string,
  taskId: string,
): CurrentBundleLoad {
