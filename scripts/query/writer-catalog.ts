...
export interface WriterCatalogPort {
  readonly writersForTable: (
    table: ProducerTableIdentity,
  ) => readonly WriterHit[];
  readonly writersForQualifiedName: (
...
export function catalogWritersFromDatasetIo(input: {
  ...
  for (const record of input.records) {
    ...
    if (identity.platform === "unknown" || identity.dataSource === "unknown")
      continue;
    ...
    rows.push({
      ...
      writeObservationId: deriveWriteObservationId(
        input.taskId,
        identity,
        record,
      ),
...
function rowToHit(row: Record<string, unknown>): WriterHit {
  return {
    ...
    writeKind:
      row.write_kind === null || row.write_kind === undefined
        ? null
        : String(row.write_kind),
    resolutionStatus:
      row.resolution_status === null || row.resolution_status === undefined
        ? null
        : String(row.resolution_status),
    physicalDataset: String(row.physical_dataset),
    partitionJson:
      row.partition_json === null || row.partition_json === undefined
        ? null
...
export function hasConfirmedWriter(
  ...
  const row = handle.db
    .prepare(
      "SELECT 1 AS present FROM table_writers WHERE table_key = ? LIMIT 1",
    )
    .get(tableKey(table)) as { present: number } | undefined;
  return row?.present === 1;
}

export function writerCatalogPort(
  handle: WriterCatalogHandle,
): WriterCatalogPort {
  return {
...
> {
  ...
  const index = new Map<
    string,
    { taskCategory: string; contentHash: string }
  >();
...
function manifestTaskContentHash(
  ...
  if (
    typeof inputPack !== "object" ||
    inputPack === null ||
    Array.isArray(inputPack)
  )
    return null;
...
function syncTaskFromFactsBundle(input: {
  ...
  try {
    status = JSON.parse(readFileSync(statusPath, "utf8")) as Record<
      string,
      unknown
    >;
  } catch {
