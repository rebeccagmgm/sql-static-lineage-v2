...
export function writeStatusFile(path: string, status: unknown): void {
  ...
  if (existsSync(backup))
    throw new Error("RECOVERY_REQUIRED: stale status backup exists");
...
export function recoverArtifactState(
  root: string,
  ...
  if (existsSync(statusBackup)) {
    if (existsSync(statusPath))
      throw new Error("RECOVERY_REQUIRED: status and status backup both exist");
  ...
  for (const stagingPath of staging) {
    if (validateBundle(stagingPath).length === 0)
      throw new Error(
        "RECOVERY_REQUIRED: valid staging directory requires inspection",
      );
  ...
  if (existsSync(recovery)) {
    ...
    if (existsSync(bundle) || errors.length > 0)
      throw new Error(
        `RECOVERY_REQUIRED: recovery directory requires inspection (${errors.join("; ")})`,
      );
  ...
  if (existsSync(statusPath)) {
    ...
    if (status.state === "ANALYZING")
      throw new Error("RECOVERY_REQUIRED: previous analysis was interrupted");
  }
}
...
export function publishArtifactBundle(options: {
  ...
  if (hadExisting) {
    const existing = JSON.parse(
      readFileSync(join(options.bundle, "manifest.json"), "utf8"),
    ) as unknown;
    const existingErrors = options.validateBundle(options.bundle);
    if (
      existingErrors.length === 0 &&
      sha256(canonicalJson(existing)) === sha256(manifestBytes)
    ) {
      rmSync(options.staging, { recursive: true, force: true });
      return {
        status: "REUSED",
        manifest_sha256: sha256(canonicalJson(existing)),
      };
    }
    if (
      existingErrors.length === 0 &&
      ...
      rmSync(options.staging, { recursive: true, force: true });
      throw new Error(
        "NON_DETERMINISTIC_OUTPUT: same analysis context produced different Bundle content",
      );
    }
  }
  const recovery = join(options.root, ".recovery");
  if (existsSync(recovery))
    throw new Error("RECOVERY_REQUIRED: recovery directory exists");
  try {
    ...
    if (errors.length)
      throw new Error(
        `published Bundle failed validation: ${errors.join("; ")}`,
      );
    if (existsSync(recovery))
      rmSync(recovery, { recursive: true, force: true });
    return {
      status: hadExisting ? "REPLACED" : "CREATED",
      manifest_sha256: manifestHash,
    };
  } catch (error) {
    if (existsSync(options.bundle))
      rmSync(options.bundle, { recursive: true, force: true });
