...
export const LEGACY_PLATFORM_TARGET_QUERY_OUTPUT =
  "PLATFORM_TARGET_QUERY_OUTPUT" as const;
export type PlatformTargetQueryOutputKind =
  | typeof PACK_DECLARED_QUERY_OUTPUT
  | typeof LEGACY_PLATFORM_TARGET_QUERY_OUTPUT;
export type OutputEvidenceKind =
  "SQL_EXPLICIT_WRITE" | PlatformTargetQueryOutputKind;

export function isPlatformTargetQueryOutputKind(
  value: unknown,
): value is PlatformTargetQueryOutputKind {
  return (
    value === PACK_DECLARED_QUERY_OUTPUT ||
    value === LEGACY_PLATFORM_TARGET_QUERY_OUTPUT
  );
}
...
export type OutcomeClass =
  "UNKNOWN" | "NOT_EVALUABLE" | "NOT_APPLICABLE" | "FAILURE";
...
export interface MachineFactsManifest {
  ...
  readonly method: {
    ...
    readonly adapter: {
      readonly name: "machine-facts-writer";
      readonly version: string;
    };
    readonly plan_adapter: {
      readonly name: "plan-adapter";
      readonly version: string;
    };
  };
...
export interface RelationEdgeRecord {
  readonly edge_id: string;
  readonly task_id: string;
  readonly statement_id: string;
  readonly from_relation_id: string;
  readonly to_relation_id: string;
...
export interface FieldExpressionRecord {
  ...
  readonly output_name_status?: string;
  readonly expression_text: string;
  /** New bundles emit display_text; legacy bundles may omit it. */ readonly display_text?: string;
  readonly source_span: unknown;
  readonly input_fields: readonly unknown[];
  readonly candidate_input_fields?: readonly unknown[];
  readonly unresolved_input_columns: readonly unknown[];
  readonly input_dependency_status: InputDependencyStatus;
  readonly expression_roles?: readonly ExpressionRoleBindingRecord[];
  readonly window_spec?: WindowSpecRecord;
...
export interface ColumnLineageRecord {
  ...
  readonly task_id: string;
  readonly statement_id: string;
  readonly from_field_id: string;
  readonly to_expression_id: string;
  readonly method: string;
  readonly resolution_provenance: string;
  readonly [key: string]: unknown;
}
export type HopCoverageState =
  "FULL_HOP" | "FLAT_ORIGIN_ONLY" | "UNKNOWN_COVERAGE" | "NOT_EVALUABLE";
export type HopProjectionStatus =
  "PROJECTED" | "PARTIAL_NATIVE" | "NOT_EVALUABLE";
...
export interface LineageHopNodeRecord {
  ...
  readonly via_relation_ids: readonly {
    relation_id: string;
    kind: "rename" | "expand";
  }[];
  readonly flow_kind: "VALUE_LINEAGE";
...
export interface TaskFactIndexRecord {
  readonly task_id: string;
  readonly logical_source_id: string;
  readonly sql_sha256: string;
...
export type MachineFactRecord =
  | StatementRecord
  | SchemaReferenceRecord
  | DatasetIoRecord
  | TaskLocalMaterializationRecord
  | RelationNodeRecord
  | RelationEdgeRecord
  | FieldExpressionRecord
  | ColumnLineageRecord
  | LineageHopRootRecord
  | LineageHopNodeRecord
  | LineageHopEdgeRecord
  | OutputFieldBindingRecord
...
export interface GenericTaskProfile {
  ...
  readonly sql_slot?: string;
  readonly sql_segments?: readonly {
