...
import {
  canonicalMachineFactsJson as canonicalJson,
  canonicalMachineFactsJsonl as canonicalJsonl,
...
import {
  deriveOutputFieldBindings,
  type WriteOutputContext,
} from "./output-field-bindings.ts";
...
function dedupeEquivalentQueryOutputs<
  T extends {
...
function hasSchemaTable(
  table: string,
  available: SchemaAvailability,
  dialect: string,
): boolean {
  const provider = available as Partial<Pick<Schema, "columnsFor">>;
  if (typeof provider.columnsFor === "function") {
    const columns = provider.columnsFor.call(
      available,
      normalizeName(table).split("."),
      dialect,
    );
...
function normalizeWrites(task: GenericTaskProfile): string[] {
  ...
  return (Array.isArray(task.writes) ? [...task.writes] : [task.writes])
    .filter(Boolean)
    .map(normalizeName);
}
...
function classifyStatement(text: string): string {
  ...
  if (extractedWrite?.writeKind === "INSERT_OVERWRITE")
...
function parseSqlWrite(text: string): string | null {
  ...
  return (
    extractSqlWrites(text).find((write) => write.writeKind !== "CTAS")
      ?.qualifiedName ?? null
  );
}

function resolveDeclaredWriteTarget(
  ...
  if (normalizedTarget.split(".").length === 1) {
    const sameTail = declared.filter((candidate) =>
      sameTableReference(candidate, normalizedTarget),
    );
...
function globalizeRelation(
  taskId: string,
  statementIndex: number,
  relation: JsonRecord,
): JsonRecord {
  const mapId = (id: string): string =>
    globalRelationId(taskId, statementIndex, id);
  const converted = stripVolatile(relation) as JsonRecord;
  converted.id = mapId(relation.id);
  if (relation.type === "read") {
    converted.read_occurrence_id = mapId(
      String(relation.read_occurrence_id ?? relation.id),
    );
    if (
      converted.read_occurrence &&
      typeof converted.read_occurrence === "object"
    ) {
...
export function mergeSchemaEvidence(
  raws: readonly JsonRecord[],
  ...
  for (const raw of raws) {
    ...
    for (const record of records) {
      ...
      if (isBetterSchemaRecord(candidate, byQualifiedName.get(key)))
        byQualifiedName.set(key, candidate);
    }
  }
  return {
    ...
    records: stableRecords([...byQualifiedName.values()], (record) =>
      normalizeName(
        String(
          record.qualified_name ?? `${record.db ?? ""}.${record.table ?? ""}`,
        ),
      ),
    ),
  };
  ...
  try {
    ...
    for (const [statementIndex, cell] of session.doc.statements.entries()) {
      ...
      const plan = parserSql.restore(
        buildPlanFacts(
          planSession.doc.statements[statementIndex] ?? cell,
          planSql.sql,
          {
...
function schemaBundleForTask(
  ...
  const records = (globalSchemaBundle.records as JsonRecord[]).filter(
    (record) => {
      const qualifiedName = String(
        record.qualified_name ?? `${record.db ?? ""}.${record.table ?? ""}`,
      ).trim();
  ...
  return mergeSchemaEvidence(
    [{ records }],
    String(globalSchemaBundle.logical_source_id),
  );
}
...
function schemaProvider(schemaBundle: JsonRecord): Schema {
  ...
  for (const record of schemaBundle.records as JsonRecord[]) {
    ...
    const table: SchemaMapping = Object.fromEntries(
      record.columns.map((column: JsonRecord) => [
        String(column.name),
        "unknown",
      ]),
    );
...
function outputColumns(relation: JsonRecord): JsonRecord[] {
  if (relation.type === "project")
    return Array.isArray(relation.expressions) ? relation.expressions : [];
  if (relation.type === "aggregate")
    return Array.isArray(relation.measures) ? relation.measures : [];
  return [];
}

export function inputDependencyStatus(
  expression: JsonRecord,
): InputDependencyStatus {
  const inputs = Array.isArray(expression.input_columns)
    ? (expression.input_columns as JsonRecord[])
    : [];
  const hasPhysical = inputs.some(
    (input) =>
      input.resolution === "PHYSICAL" &&
      Array.isArray(input.physical) &&
      input.physical.length > 0,
  );
  const hasDerived = inputs.some(
    (input) => input.resolution === "DERIVED_OUTPUT",
  );
  ...
  if (hasPhysical && (hasUnresolved || hasDerived || hasSqlCandidate))
    return "PARTIAL";
...
function physicalTablesIn(
  ...
  if (Array.isArray(object.physical)) {
    for (const physical of object.physical as JsonRecord[]) {
      if (physical && typeof physical.table === "string")
        result.add(normalizeName(physical.table));
    }
  }
  if (object.type === "read" && typeof object.table === "string")
    result.add(normalizeName(object.table));
  for (const [key, child] of Object.entries(object))
    if (key !== "physical") physicalTablesIn(child, result);
  return result;
}
...
function fieldInputsForRefs(
  ...
  return {
    inputFields: [
      ...new Map(physical.map((item) => [item.field_id, item])).values(),
    ],
    candidateFields: [
      ...new Map(candidates.map((item) => [item.field_id, item])).values(),
    ],
  };
}

function windowSpecRecord(
  logicalSourceId: string,
  expression: JsonRecord,
  ...
  return {
    ...
    input_bindings: (spec.input_bindings as JsonRecord[]).map((binding) => {
      const refs = Array.isArray(binding.input_columns)
      ...
      return {
        ...
        unresolved_input_columns: unresolvedInputColumns({
          input_columns: refs,
        }),
        input_dependency_status: inputDependencyStatus({ input_columns: refs }),
        ...(binding.role === "WINDOW_ORDER"
          ? {
              direction: binding.direction ?? "ASC",
              nulls: binding.nulls ?? "UNSPECIFIED",
            }
          : {}),
      };
    }),
    ...(frame
      ? {
          frame: {
            ...
            input_fields: fieldInputsForRefs(
              logicalSourceId,
              Array.isArray(frame.input_columns) ? frame.input_columns : [],
            ).inputFields,
            unresolved_input_columns: unresolvedInputColumns({
              input_columns: Array.isArray(frame.input_columns)
                ? frame.input_columns
                : [],
            }),
            input_dependency_status: inputDependencyStatus({
              input_columns: Array.isArray(frame.input_columns)
                ? frame.input_columns
                : [],
            }),
  ...
  if (item.field === "physical") {
    ...
    if (String(item.reason ?? "").includes("followColumn 无来源")) {
      return {
        ...
        reason_code: "PHYSICAL_FIELD_UNRESOLVED",
      };
    }
  ...
  if (item.field === "output_columns") {
    if (statementType === "CREATE_TABLE" && expressions.length === 0) {
      return {
        ...
        reason_code: "NON_QUERY_OUTPUT_NOT_APPLICABLE",
      };
    }
    if (!schemaAvailable) {
      return {
    ...
    if (
      ...
      return {
        outcome_class: "UNKNOWN",
        reason_code: "STAR_EXPANSION_UNRESOLVED",
      };
    }
    if (
      expressions.some(
        (expression) =>
          expression.output === "?" ||
          expression.output_name_status === "ANONYMOUS_EXPRESSION",
      )
    ) {
      return {
        outcome_class: "UNKNOWN",
        reason_code: "ANONYMOUS_OUTPUT_NAME_UNRESOLVED",
      };
    }
  }
...
const SQL_CANDIDATE_SAFE_RESOLUTIONS = new Set([
  "PHYSICAL",
  "SQL_CANDIDATE",
  "DERIVED_OUTPUT",
]);
...
function canRepresentMissingSchemaAsSqlCandidate(
  ...
  return (
    safe &&
    missing.size > 0 &&
    [...missing].every((table) => candidateTables.has(table))
  );
}
...
function writeStatus(taskRoot: string, status: AnalysisStatus): void {
  ...
  if (existsSync(backup))
    throw new Error("RECOVERY_REQUIRED: stale analysis-status backup exists");
...
function snapshot(
  ...
  const path = join(
    ...
    "snapshots",
    kind,
    `${hash}.${kind === "sql" ? "sql" : "json"}`,
  );
  mkdirSync(dirname(path), { recursive: true });
  if (existsSync(path)) {
    if (fileHash(path) !== hash)
      throw new Error(`snapshot hash collision: ${relativeRoot(root, path)}`);
  } else {
...
function planRecords(
  ...
  const relationIds = new Set(
    plan.relations.map((relation) =>
      globalRelationId(task.task_id, statementIndex, relation.id),
    ),
  );
  ...
  for (const table of plan.physical_inputs) {
    const readOccurrences = planRelations
      .filter(
        (relation) =>
          relation.type === "read" &&
          relation.is_cte !== true &&
          normalizeName(String(relation.table ?? "")) === normalizeName(table),
      )
      .map((relation) => ({
        occurrence_id: globalRelationId(
          task.task_id,
          statementIndex,
          String(relation.read_occurrence_id ?? relation.id),
        ),
        relation_id: globalRelationId(
          task.task_id,
          statementIndex,
          String(relation.id),
        ),
        ...
        source_span:
          relation.read_occurrence?.source_span ?? relation.span ?? null,
      }));
    reads.push({
  ...
  for (const item of plan.unknowns) {
    const relation = planRelations.find(
  ...
  for (const relation of planRelations) {
    ...
    const explicitPhysicalUnknown = explicitPhysicalUnknowns.has(
      String(relation.id),
    );
  ...
  for (const localRelation of plan.relations as JsonRecord[]) {
    const relation = globalizeRelation(
      task.task_id,
      statementIndex,
      localRelation,
    );
    ...
    const node: JsonRecord = {
      ...
      source_text: spanValid(sourceSpan, sql)
        ? sql.slice(sourceSpan.start, sourceSpan.end)
        : null,
      provenance:
        relation.provenance === "extracted" ? "SQL_PLAN" : "PARTIAL_SQL_PLAN",
      relation,
    };
    relations.push(node);
    const refs = [
    ...
    for (const [role, expressions] of [
      [
        "PROJECT_EXPRESSION",
        relation.type === "project" ? outputColumns(relation) : [],
      ],
      [
        "AGGREGATE_MEASURE",
        relation.type === "aggregate" ? outputColumns(relation) : [],
      ],
    ] as const) {
      for (const [ordinal, expression] of expressions.entries()) {
        ...
        fields.push({
          ...
          input_dependency_status: inputDependencyStatus({
            input_columns: effectiveInputRefs,
          }),
          expression_roles: (
            expression.expression_roles as JsonRecord[] | undefined
          )?.map((role) => {
            const roleRefs = Array.isArray(role.input_columns)
              ? role.input_columns
              : [];
  ...
  const relationNodeById = new Map(
    relations.map((node) => [String(node.relation_id), node]),
  );
  const leafRelationIds = (
    relationId: string,
    ...
    return (
      Array.isArray(node.relation?.branches) ? node.relation.branches : []
    ).flatMap((branch: string) => leafRelationIds(String(branch), nextActive));
  };
  for (const node of relations.filter(
    (candidate) => candidate.relation_type === "setop",
  )) {
    const outputNames = Array.isArray(node.relation?.output_columns)
      ? node.relation.output_columns.map(String)
      : [];
    const leaves = new Set(leafRelationIds(String(node.relation_id)));
    for (const [ordinal, outputName] of outputNames.entries()) {
      const branchExpressions = fields.filter(
        (field) =>
          leaves.has(String(field.relation_id)) && field.ordinal === ordinal,
      );
      if (branchExpressions.length === 0) continue;
      const inputFields = [
        ...new Map(
          branchExpressions
            .flatMap((field) => field.input_fields ?? [])
            .map((field: JsonRecord) => [String(field.field_id), field]),
        ).values(),
      ];
      const candidateFields = [
        ...new Map(
          branchExpressions
            .flatMap((field) => field.candidate_input_fields ?? [])
            .map((field: JsonRecord) => [String(field.field_id), field]),
        ).values(),
      ];
      const unresolvedColumns = branchExpressions.flatMap(
        (field) => field.unresolved_input_columns ?? [],
      );
      const expressionId = `${String(node.relation_id)}:expression:setop_output:${ordinal}`;
      const dependencyStatus: InputDependencyStatus =
        inputFields.length > 0
          ? candidateFields.length > 0 || unresolvedColumns.length > 0
            ? "PARTIAL"
            : "PHYSICAL"
...
function validateJsonSchema(
  ...
  if (
    ...
    errors.push(
      `${path}: expected ${Array.isArray(type) ? type.join("|") : type}, got ${actual}`,
    );
    return errors;
  }
...
function outputPath(bundleDir: string, path: string): string | null {
  ...
  return relativePath === path && !path.startsWith("/") && !path.includes("..")
    ? resolved
    : null;
}
...
function snapshotReference(
  ...
  return relative(root, resolved).replace(/\\/g, "/") === reference
    ? resolved
    : null;
}
...
export function validateBundle(bundleDir: string): string[] {
  ...
  try {
    ...
    const schemaErrors = validateJsonSchema(
      manifest,
      json<JsonRecord>(schemaPath),
    );
    for (const error of schemaErrors) errors.push(`manifest schema: ${error}`);
  } catch (error) {
    errors.push(
      `manifest schema validation failed: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  let recordSchemas: JsonRecord = {};
  try {
    recordSchemas = (json<JsonRecord>(
      join(workspace, "schemas", "machine-facts-records.schema.json"),
    ).properties ?? {}) as JsonRecord;
  } catch (error) {
    errors.push(
      `record schema validation failed: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  if (manifest.schema_version !== MACHINE_FACTS_CONTRACT_VERSION)
    errors.push("unsupported manifest schema_version");
  if (manifest.status !== "SUCCESS")
    errors.push("manifest status is not SUCCESS");
  for (const output of manifest.outputs ?? []) {
    ...
    if (hashJsonlStore(path) !== output.content_sha256)
      errors.push(`hash mismatch ${output.path}`);
    try {
      const rows = readJsonlRecords(path);
      if (rows.length !== output.row_count)
        errors.push(`row count mismatch ${output.path}`);
      ...
      errors.push(
        `invalid JSONL ${output.path}: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  for (const required of REQUIRED_DATASETS) {
    if (
      !(manifest.outputs ?? []).some(
        (output) =>
          output && typeof output === "object" && output.path === required,
      )
    )
      errors.push(`required output not declared ${required}`);
  }
  const root = rootForBundle(bundleDir);
  const sqlPath = snapshotReference(
    root,
    manifest.inputs.sql_snapshot,
    "sql",
  ...
  if (
    !sqlPath ||
    !existsSync(sqlPath) ||
    fileHash(sqlPath) !== manifest.inputs.sql_sha256
  )
  ...
  if (
    ...
    fileHash(schemaPath) !== manifest.inputs.schema_bundle_sha256
  )
  ...
  if (!existsSync(sourceArtifactPath))
    errors.push("source-artifact.json is missing");
  else {
    try {
      const sourceArtifact = json<JsonRecord>(sourceArtifactPath);
      const sourceSchema = recordSchemas["source-artifact.json"] as
        JsonRecord | undefined;
      if (sourceSchema)
      ...
      errors.push(
        `source-artifact.json is invalid: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  const sql =
    sqlPath && existsSync(sqlPath) ? readFileSync(sqlPath, "utf8") : "";
  const statements = readJsonlForValidation(
    join(bundleDir, "statements.jsonl"),
    errors,
  );
  ...
  for (const statement of statements) {
    if (statement.task_id !== manifest.task_id)
      errors.push(`statement task isolation failed ${statement.statement_id}`);
  }
  for (const statement of statements) {
    if (
  ...
  const relationNodes = readJsonlForValidation(
    join(bundleDir, "relation-nodes.jsonl"),
    errors,
  );
  const relationIds = new Set(relationNodes.map((node) => node.relation_id));
  for (const edge of readJsonlForValidation(
    join(bundleDir, "relation-edges.jsonl"),
    errors,
  )) {
    if (
      !relationIds.has(edge.from_relation_id) ||
      !relationIds.has(edge.to_relation_id)
    )
      errors.push(`relation endpoint missing ${edge.edge_id}`);
  }
  const expressions = readJsonlForValidation(
    join(bundleDir, "field-expression-nodes.jsonl"),
    errors,
  );
  ...
  const columnLineage = readJsonlForValidation(
    join(bundleDir, "column-lineage-edges.jsonl"),
    errors,
  );
  ...
  const datasetIo = readJsonlForValidation(
    join(bundleDir, "dataset-io.jsonl"),
    errors,
  );
  const unknownRecords = readJsonlForValidation(
    join(bundleDir, "unknowns.jsonl"),
    errors,
  );
  const writeObservations = new Map(
    datasetIo
      .filter(
        (record) =>
      ...
      .map((record) => [record.write_observation_id, record]),
  );
  const expectedInputPackSqlSha256 =
    typeof manifest.inputs.input_pack?.sql_sha256 === "string"
  ...
  for (const write of writeObservations.values()) {
    ...
    if (write.provenance !== "PLATFORM_TARGET")
      errors.push(
        `pack-declared write provenance is invalid ${write.write_observation_id}`,
      );
    if (
      typeof write.source_sql_sha256 !== "string" ||
      !/^[a-f0-9]{64}$/.test(write.source_sql_sha256)
    ) {
      errors.push(
        `pack-declared write source SQL hash is invalid ${write.write_observation_id}`,
      );
    } else if (write.source_sql_sha256 !== expectedInputPackSqlSha256) {
      errors.push(
        `pack-declared write source SQL hash does not match manifest ${write.write_observation_id}`,
      );
    }
  }
  const bindingOrdinals = new Set<string>();
  const bindings = readJsonlForValidation(
    join(bundleDir, "output-field-bindings.jsonl"),
    errors,
  );
  for (const binding of bindings) {
    if (!expressionIds.has(binding.expression_id))
      errors.push(
        `output binding expression endpoint missing ${binding.binding_id}`,
      );
    if (binding.binding_status !== "RESOLVED")
      errors.push(`output binding must be resolved ${binding.binding_id}`);
    if (
      !Number.isInteger(binding.source_ordinal) ||
      !Number.isInteger(binding.target_ordinal)
    )
    ...
    if (!write)
      errors.push(
        `output binding write observation endpoint missing ${binding.binding_id}`,
      );
    else {
      if (
        binding.task_id !== manifest.task_id ||
        write.task_id !== manifest.task_id
      )
        errors.push(
          `output binding task isolation failed ${binding.binding_id}`,
        );
      if (
        binding.write_kind !== write.write_kind ||
        binding.write_statement_id !== write.write_statement_id ||
        binding.statement_id !== write.statement_id ||
        binding.query_producer_statement_id !==
          (write.query_producer_statement_id ?? null)
      )
        errors.push(
          `output binding write identity mismatch ${binding.binding_id}`,
        );
      if (binding.evidence_kind === PACK_DECLARED_QUERY_OUTPUT) {
        if (
          typeof binding.source_sql_sha256 !== "string" ||
          binding.source_sql_sha256 !== write.source_sql_sha256 ||
          binding.source_sql_sha256 !== expectedInputPackSqlSha256
        ) {
          errors.push(
            `pack-declared binding source SQL hash mismatch ${binding.binding_id}`,
          );
        }
      }
      const ordinalKey = `${binding.write_observation_id}:${binding.source_ordinal}`;
      if (bindingOrdinals.has(ordinalKey))
        errors.push(`duplicate output binding producer ordinal ${ordinalKey}`);
  ...
  for (const write of writeObservations.values()) {
    ...
    const writeGaps = unknownRecords.filter(
      (unknown) =>
        unknown.write_observation_id === write.write_observation_id &&
        Array.isArray(unknown.uncovered_ordinals),
    );
    ...
    for (const ordinal of producerOrdinals) {
      if (expected.has(ordinal))
        errors.push(
          `duplicate producer ordinal ${write.write_observation_id}:${ordinal}`,
        );
    ...
    const addDisposition = (ordinal: number, source: string): void => {
      if (!expected.has(ordinal)) {
        errors.push(
          `disposition references unknown producer ordinal ${write.write_observation_id}:${ordinal}`,
        );
        return;
      }
      dispositions.set(ordinal, (dispositions.get(ordinal) ?? 0) + 1);
      if ((dispositions.get(ordinal) ?? 0) > 1)
        errors.push(
          `producer ordinal has multiple dispositions ${write.write_observation_id}:${ordinal} (${source})`,
        );
    };
    for (const binding of writeBindings)
      addDisposition(binding.source_ordinal, `binding:${binding.binding_id}`);
    for (const gap of writeGaps)
      for (const ordinal of gap.uncovered_ordinals as unknown[])
        if (typeof ordinal === "number" && Number.isInteger(ordinal))
          addDisposition(ordinal, `gap:${gap.unknown_id}`);
    for (const ordinal of expected)
      if ((dispositions.get(ordinal) ?? 0) !== 1)
        errors.push(
          `producer ordinal does not have exactly one disposition ${write.write_observation_id}:${ordinal}`,
        );
    if (
      write.producer_enumeration_status === "NOT_EVALUABLE" &&
      !unknownRecords.some(
        (unknown) =>
          unknown.write_observation_id === write.write_observation_id &&
          unknown.reason_code === "PRODUCER_OUTPUT_ENUMERATION_NOT_EVALUABLE",
      )
    ) {
      errors.push(
        `field-producing Write lacks producer enumeration gap ${write.write_observation_id}`,
      );
    }
  }
  return [...new Set(errors)];
...
function readJsonlForValidation(path: string, errors: string[]): JsonRecord[] {
  try {
    ...
    errors.push(
      `invalid JSONL ${basename(path)}: ${error instanceof Error ? error.message : String(error)}`,
    );
    return [];
  }
}
...
function reusableTaskResult(
  ...
  if (
    ...
    status.requested.analysis_config_sha256 !==
      requested.analysis_config_sha256 ||
  ...
  if (
    ...
    manifest.inputs.analysis_config_sha256 !==
      requested.analysis_config_sha256 ||
...
function buildTaskBundle(
  ...
  const schemaRefs: SchemaReferenceRecord[] = (
    schemaBundle.records as JsonRecord[]
  ).map((record, index) => ({
  ...
  for (const [statementIndex, cell] of session.doc.statements.entries()) {
    ...
    while (
      significantStart < span.end &&
      /\s/.test(sql[significantStart] ?? "")
    )
      significantStart += 1;
    const segment = task.sql_segments?.find(
      (candidate) =>
        significantStart >= candidate.start && span.end <= candidate.end,
    );
    const localOrdinal = segment
      ? (segmentOrdinals.get(segment.slot) ?? 0)
      : statementIndex;
    ...
    const extractedWrite =
      parsedWrite === null
        ? undefined
        : extractSqlWrites(rawSql).find((write) =>
            sameTableReference(write.qualifiedName, parsedWrite),
          );
    const writeTarget =
      parsedWrite === null
        ? null
        : resolveDeclaredWriteTarget(task, parsedWrite);
    ...
    const hasActionableUnknown = plan.unknowns.some((item) => {
      const relation = (plan.relations as JsonRecord[]).find(
        (candidate) => candidate.id === item.node_id,
      );
      return (
        classifyPlanUnknown(
    ...
    statements.push({
      ...
      raw_sql: rawSql,
      parse_status:
        cell.errors > 0 || hasActionableUnknown ? "PARTIAL" : "SUCCESS",
    ...
    const rootRelationIds = new Set(
      plan.roots.map((rootId) =>
        globalRelationId(task.task_id, statementIndex, rootId),
      ),
    );
    const producerExpressions = records.fields.filter((expression) =>
      rootRelationIds.has(expression.relation_id),
    );
    const producerOrdinals = producerExpressions
      .map((expression) => expression.ordinal)
      .sort((left, right) => left - right);
    const producerComplete =
      producerOrdinals.length > 0 &&
      producerOrdinals.every((ordinal, index) => ordinal === index);
    if (
      !parsedWrite &&
      (statementType === "SELECT" || statementType === "WITH_QUERY") &&
      producerComplete
    ) {
    ...
    if (parsedWrite) {
      const sourceStatementBase =
        segment === undefined ? span.start : span.start - segment.start;
      const sourceStatementStart =
        extractedWrite === undefined
          ? segment === undefined
            ? significantStart
            : significantStart - segment.start
          : sourceStatementBase + extractedWrite.statementSpan.start;
      const sourceStatementEnd =
        extractedWrite === undefined
          ? undefined
          : sourceStatementBase + extractedWrite.statementSpan.end;
      const partitionEvidence = task.sql_write_partition_evidence?.find(
        (item) =>
          writeTarget !== null &&
          sameTableReference(item.target, writeTarget) &&
          ...
          (item.statement_start !== undefined
            ? item.statement_start === sourceStatementStart &&
              (item.statement_end === undefined ||
                item.statement_end === sourceStatementEnd)
            : item.statement_ordinal === undefined ||
              item.statement_ordinal === localOrdinal),
      );
      const hasCtasBoundary =
        statementType === "CREATE_TABLE" &&
        /\bAS\s+(?:SELECT|WITH)\b/i.test(rawSql);
      const writeKind =
        statementType === "CREATE_TABLE"
          ? hasCtasBoundary
            ? "CTAS"
            : "CREATE_TABLE"
          : statementType;
      const fieldProducing =
        hasCtasBoundary ||
        statementType === "INSERT_OVERWRITE" ||
        statementType === "INSERT_INTO";
      const producerEnumerationStatus =
        fieldProducing && producerComplete
          ? "COMPLETE"
          : fieldProducing
            ? "NOT_EVALUABLE"
            : "NOT_APPLICABLE";
      const writeObservationId = sqlParsedWriteObservationId(
        task.task_id,
        statementIndex,
      );
      datasetIo.push({
  ...
  if (task.platform_target_query_output && !hasExplicitPlatformTargetWrite) {
    ...
    if (
      ...
      /^[a-f0-9]{64}$/.test(sourceSqlSha256)
    ) {
      const candidate = uniqueQueryOutputCandidates[0]!;
      const writeObservationId = platformTargetWriteObservationId(
        task.task_id,
      ...
      datasetIo.push({
        ...
        producer_ordinals: candidate.expressions.map(
        ...
        source_as_boundary: {
          ...
          statement_span:
            statements.find(
              (statement) => statement.statement_id === candidate.statementId,
            )?.span ?? null,
        },
      });
      writeContexts.push({
      ...
      unknowns.push({
        ...
        message:
          "platform target requires a valid original Input Pack SQL SHA-256",
        subject: target,
      });
    } else {
      unknowns.push({
  ...
  const fieldProducingDatasets = new Set(
    datasetIo
      .filter(
        (record) =>
          record.direction === "WRITE" && record.field_producing === true,
      )
      .map((record) => normalizeName(record.physical_dataset)),
  );
  const declaredWritesWithoutBindings = normalizeWrites(task).filter(
    (write) =>
      fieldProducingDatasets.has(write) &&
      !outputBindings.some(
        (binding) => normalizeName(binding.target_dataset) === write,
      ),
  );
  if (declaredWritesWithoutBindings.length > 0) {
    unknowns.push({
      ...
      message:
        "Profile declared output has no unambiguous SQL output field binding",
      subject: declaredWritesWithoutBindings.join(","),
    });
  }
  const unknownsByOutcome = Object.fromEntries(
    (["UNKNOWN", "NOT_EVALUABLE", "NOT_APPLICABLE", "FAILURE"] as const).map(
      (outcome) => [
        outcome,
  ...
  for (const [name, records, schemaVersion] of [
    ...
    [
      ...
      taskLocalMaterializations,
      "machine-facts-task-local-materializations-v1",
    ],
  ...
  const manifest: MachineFactsManifest = {
    ...
    inputs: {
      ...
      ...(task.input_pack_provenance
        ? { input_pack: task.input_pack_provenance }
        : {}),
    },
    method: {
      dialect: profile.dialect,
      ...
      adapter: {
        name: "machine-facts-writer",
        version: MACHINE_FACTS_ADAPTER_VERSION,
      },
    ...
    counts: {
      ...
      lineage_hop_projected_roots: hopRoots.filter(
        (root) => root.projection_status === "PROJECTED",
      ).length,
      lineage_hop_partial_roots: hopRoots.filter(
        (root) => root.projection_status === "PARTIAL_NATIVE",
      ).length,
      lineage_hop_not_evaluable_roots: hopRoots.filter(
        (root) => root.projection_status === "NOT_EVALUABLE",
      ).length,
    ...
    gates: {
      ...
      lineage_hop_endpoints: false,
      lineage_hop_acyclic: false,
      lineage_hop_status_truth_table: false,
      lineage_hop_origin_conservation: false,
    },
    boundaries: {
...
export function runTask(
  ...
  const reusable = reusableTaskResult(
    taskRoot,
    task.task_id,
    logicalSourceId,
    requested,
  ...
  writeStatus(taskRoot, {
    ...
    current_manifest_sha256:
      readStatus(taskRoot)?.current_manifest_sha256 ?? null,
  });
  let staging: string | null = null;
  try {
    const built = buildTaskBundle(
      task,
      profile,
      logicalSourceId,
      root,
...
function readTaskFactIndexSchema(): {
  ...
  try {
    ...
    return {
      schema: null,
      failure: `task-fact-index schema unavailable: ${error instanceof Error ? error.message : String(error)}`,
    };
  }
}
...
function taskFactIndexRecord(
  ...
  try {
    ...
    return {
      failure: `${taskId}: invalid analysis-status.json (${error instanceof Error ? error.message : String(error)})`,
    };
  }
  if (!status || status.state !== "SUCCESS")
    return {
      failure:
        status?.state === "FAILED"
          ? `${taskId}: ${status.failure?.reason_code ?? "FAILED"}`
          : undefined,
    };
  const manifestPath = join(taskRoot, "bundle", "manifest.json");
  if (!existsSync(manifestPath))
    return { failure: `${taskId}: manifest missing` };
  let manifest: MachineFactsManifest;
  try {
    manifest = json<MachineFactsManifest>(manifestPath);
  } catch (error) {
    return {
      failure: `${taskId}: invalid manifest (${error instanceof Error ? error.message : String(error)})`,
    };
  }
  const manifestHash = sha256(canonicalJson(manifest));
  if (
...
export function rebuildIndex(root: string): ProfileRunResult["index"] {
  ...
  if (existsSync(tasksRoot)) {
    for (const taskId of readdirSync(tasksRoot)) {
      ...
      const requestedStatus = status.requested as
        AnalysisStatus["requested"] | undefined;
      const statusMatchesManifest =
        ...
        requestedStatus.schema_bundle_sha256 ===
          manifest.inputs.schema_bundle_sha256 &&
        requestedStatus.analysis_config_sha256 ===
          manifest.inputs.analysis_config_sha256 &&
        requestedStatus.dialect === manifest.method.dialect;
      if (
        !statusMatchesManifest ||
        sha256(canonicalJson(manifest)) !== status.current_manifest_sha256
      ) {
      ...
      const indexErrors = indexSchema
        ? validateJsonSchema(
            candidate,
            indexSchema,
            `task-fact-index.jsonl[${records.length}]`,
          )
  ...
  writeFileSync(
    path,
    canonicalJsonl(stableRecords(records, (record) => String(record.task_id))),
    "utf8",
  );
...
export function updateIndexIncrementally(
  ...
  for (const result of options.taskResults) {
    ...
    if (result.state !== "SUCCESS") {
      if (result.failures.length > 0)
        failures.push(
          `${result.task_id}: ${result.failures.map((failure) => failure.message).join("; ")}`,
        );
      continue;
    }
    ...
    if (
      ...
      result.manifest_sha256 !== candidate.record.manifest_sha256
    ) {
    ...
    const indexErrors = indexSchemaResult.schema
      ? validateJsonSchema(
          candidate.record,
          indexSchemaResult.schema,
          `task-fact-index.jsonl[${records.size}]`,
        )
      : ["task-fact-index schema unavailable"];
  ...
  writeFileSync(
    path,
    canonicalJsonl(
      stableRecords([...records.values()], (record) => String(record.task_id)),
    ),
    "utf8",
  );
...
export function processProfile(
  ...
  if (!sourceIdOverride && !profile.logical_source_id)
    throw new Error("logical_source_id is required");
  const logicalSourceId = safeSegment(
    sourceIdOverride ?? profile.logical_source_id!,
    "logical_source_id",
  );
  const taskIds = profile.tasks.map((task) => task.task_id);
  if (new Set(taskIds).size !== taskIds.length)
    throw new Error("profile task_id values must be unique");
  ...
  const configuredEvidence = Array.isArray(profile.schema_evidence)
    ...
    : profile.schema_evidence
      ? [profile.schema_evidence]
      : [];
  const evidencePaths = configuredEvidence.map((path) =>
    resolve(workspace, path),
  );
  if (
    evidencePaths.length === 0 ||
    evidencePaths.some((path) => !existsSync(path))
  )
...
function parseArgs(args: string[]): {
  ...
  return {
    profile: value(
      "--profile",
      "cases/indicator-journey-rgstcomp-mthend/processing-graph-profile.json",
    ),
    output: value("--output", "machine-facts"),
    sourceId: args.includes("--source-id")
      ? value("--source-id", "")
      : undefined,
  };
}
...
function deriveTaskLocalMaterializations(
  ...
  const writes = datasetIo.filter(
    (record) =>
      record.direction === "WRITE" &&
      typeof record.write_observation_id === "string",
  );
  const writeByObservation = new Map(
    writes.map((write) => [String(write.write_observation_id), write]),
  );
  const bindingsByObservation = new Map<string, OutputFieldBindingRecord[]>();
  for (const binding of outputBindings) {
    const values =
      bindingsByObservation.get(binding.write_observation_id) ?? [];
    values.push(binding);
    bindingsByObservation.set(binding.write_observation_id, values);
  }
  const fieldsByStatement = new Map<
    string,
    Map<
      string,
      {
        ...
        readonly expressionIds: Set<string>;
      }
    >
  >();
  for (const expression of expressions) {
    ...
    for (const raw of expression.input_fields) {
      const input =
        typeof raw === "object" && raw !== null ? (raw as JsonRecord) : null;
  ...
  for (const read of reads) {
    ...
    const priorWrites = writes
      .filter((write) => {
        const writeStatementId = String(
          write.write_statement_id ?? write.statement_id ?? "",
        );
        const writeStatementIndex = statementIndexes.get(writeStatementId);
        return (
          normalizeName(String(write.physical_dataset ?? "")) ===
            physicalDataset &&
          writeStatementIndex !== undefined &&
          writeStatementIndex < readStatementIndex
        );
      })
      .sort(
        (left, right) =>
          (statementIndexes.get(
            String(left.write_statement_id ?? left.statement_id ?? ""),
          ) ?? 0) -
            (statementIndexes.get(
              String(right.write_statement_id ?? right.statement_id ?? ""),
            ) ?? 0) ||
          compareText(
            String(left.write_observation_id),
            String(right.write_observation_id),
          ),
      );
    ...
    for (const field of fields) {
      const candidates = priorWrites.flatMap((write) =>
        (
          bindingsByObservation.get(String(write.write_observation_id)) ?? []
        ).filter(
          (binding) =>
            normalizeName(binding.target_dataset) === physicalDataset &&
            normalizeName(binding.target_field) === field.column,
        ),
      );
      ...
      if (priorWrites.length === 1 && candidates.length === 1) {
        ...
        const writeStatementId = String(
          write?.write_statement_id ??
            write?.statement_id ??
            binding.write_statement_id,
        );
        const writeStatementIndex = statementIndexes.get(writeStatementId);
      ...
      records.push({
        ...
        read_statement_id: readStatementId,
        write_statement_index:
          priorWrites.length === 1
            ? (statementIndexes.get(
                String(
                  priorWrites[0]!.write_statement_id ??
                    priorWrites[0]!.statement_id ??
                    "",
                ),
              ) ?? -1)
            : -1,
        ...
        read_expression_ids: [...field.expressionIds].sort(compareText),
        status:
          candidates.length > 1 || priorWrites.length > 1
            ? "AMBIGUOUS"
            : "UNRESOLVED",
        provenance: "SAME_TASK_SQL_WRITE_READ",
        evidence_refs: evidenceRefs,
  ...
  return [...records].sort((left, right) =>
    compareText(left.bridge_id, right.bridge_id),
  );
}
