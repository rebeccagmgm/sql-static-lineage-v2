...
import { physicalFieldForTable } from "./physical-field-resolver.ts";
import {
  type FactsPolicy,
...
export interface PhysicalFieldExpanderTaskPackLookup {
  readonly get: (taskId: string) => PhysicalFieldExpanderTaskPack | undefined;
}
...
type ClassifiedProducerRole =
  "PRIMARY" | "ADDITIONAL" | "UNKNOWN" | "CANDIDATE";
...
function tableLineageDecision(
  ...
  const node = (
    Array.isArray(tableLineage.taskNodes) ? tableLineage.taskNodes : []
  )
    .map(asRecord)
...
function bundleIndexesFor(load: CurrentBundleLoad): BundleIndexes {
  ...
  for (const edge of load.records["relation-edges.jsonl"] ?? []) {
    ...
    if (!to || !from) continue;
    incomingRelations.set(to, [...(incomingRelations.get(to) ?? []), from]);
  }
...
function selectBridges(
  ...
  for (const bridge of matching) {
    const occurrenceId = nonEmpty(
      asRecord(bridge.readOccurrence)?.occurrenceId,
    );
    if (!occurrenceId)
  ...
  if (producerSignatures.size === 1)
    return { classified: true, ambiguous: false, selected: matching, matching };
  const qualifiers = expressionQualifiersForColumn(
    expressionText,
    field.column,
  );
...
function hasTaskEdge(
  ...
  return [
    ...(Array.isArray(tableLineage.producerBridges)
      ? tableLineage.producerBridges
      : []),
    ...(Array.isArray(tableLineage.scheduleEdges)
...
function producerRelationMatchesField(
  ...
  if (!hasTaskEdge(context.tableLineage, consumerTaskId, producerTaskId))
    return false;
  const bridgeMatch = (
    Array.isArray(context.tableLineage.producerBridges)
  ...
  return scheduleReadFallbackMatches(
    context,
    consumerTaskId,
    producerTaskId,
    field,
...
function scheduleReadFallbackMatches(
  ...
  if (decision?.primary.length !== 1 || decision.primary[0] !== producerTaskId)
    return false;
  const scheduleMatch = (
    Array.isArray(context.tableLineage.scheduleEdges)
  ...
  const eligibleReads = (
    Array.isArray(context.tableLineage.readEdges)
...
function canonicalRelationIdentity(value: unknown): string | null {
  ...
  return normalizeName(
    marker >= 0
      ? withoutQueryPrefix.slice(marker + ":relation:".length)
      : withoutQueryPrefix,
  );
}
...
function statementIndexForRelation(
  ...
  return String(relation.relation_id ?? "").match(
    /:statement:(\d+):relation:/,
  )?.[1]
    ? Number(
        String(relation.relation_id).match(/:statement:(\d+):relation:/)?.[1],
      )
    : null;
}
...
function validateConsumerReadOccurrence(
  ...
  if (
    ...
    return {
      valid: false,
      reason: "CONSUMER_READ_OCCURRENCE_EVIDENCE_MISMATCH",
    };
  ...
  if (
    !Object.prototype.hasOwnProperty.call(load.records, "relation-nodes.jsonl")
  )
  ...
  const matchingRead = (load.records["relation-nodes.jsonl"] ?? []).find(
    (node) => {
      ...
      const relationIdentityMatches = sameRelationIdentity(
        actualRelationId,
        readRelationId,
      );
      ...
      const legacyIdentity =
        ...
        normalizeName(String(relation?.scope_id ?? "")).split(".")[0] ===
          normalizeName(
            String(canonicalRelationIdentity(relationPath[0]) ?? "").split(
              ".",
            )[0],
          );
      ...
      const identityProven = occurrenceIdentityMatches
    ...
    if (Array.isArray(record.read_occurrences))
      return record.read_occurrences.some((rawOccurrence) => {
        ...
        return (
          ...
          sameRelationIdentity(
            readOccurrence.relation_id,
            matchingRead.relation_id,
          )
        );
      });
  ...
  const actualScope = nonEmpty(
    relation?.scope_id ?? asRecord(relation?.read_occurrence)?.scope_id,
  );
    ...
    .filter((binding) => {
      ...
      return (
        ...
        (evidenceMode === "LEGACY_COMPAT" ||
          bindingWriteStatementId === null ||
          bindingWriteStatementId === writeStatementId) &&
        (evidenceMode === "LEGACY_COMPAT" ||
          bindingStatementId === null ||
          bindingStatementId === writeRecordStatementId) &&
...
function hasConcretePartitionScope(artifactWrite: JsonRecord): boolean {
  ...
  const assignments = (
    Array.isArray(artifactWrite.partition) ? artifactWrite.partition : []
  )
    .map(asRecord)
...
function sqlParseSpan(
  ...
  for (const rawEvidence of Array.isArray(write.evidence)
    ? write.evidence
    : []) {
...
function strictWriteMatchesArtifact(
  ...
  if (observationKind === "SQL_EXPLICIT_WRITE") {
    ...
    if (
      expectedKind !== null &&
      normalizeName(String(producerRecord.write_kind ?? "")) !==
        normalizeName(expectedKind)
    )
      return false;
    ...
    const boundary = asRecord(
      producerRecord.source_as_boundary,
    )?.statement_span;
...
function legacySqlParseStart(write: JsonRecord): number | null {
  for (const rawEvidence of Array.isArray(write.evidence)
    ...
    const start = String(evidence?.locator ?? "").match(
      /#char=(\d+)(?:-\d+)?$/,
    )?.[1];
...
function legacyProducerWriteProof(
  ...
  const artifactWrites = edges.flatMap((edge) =>
    (Array.isArray(edge.writes) ? edge.writes : [])
      .map(asRecord)
      .filter((write): write is JsonRecord => write !== null),
  );
...
function strictProducerWriteProof(
  ...
  if (artifactWrites.length === 0) {
    ...
    const statementId = nonEmpty(
      record.write_statement_id ?? record.statement_id,
    );
  ...
  for (const artifactWrite of artifactWrites) {
    ...
    if (!span) continue;
    const writeKey =
      writeObservationIds(artifactWrite).join("\u0000") || "<unbound>";
    const spanKey = `${span.start}:${span.end}`;
    exactSpanKeys.set(
      writeKey,
      new Set([...(exactSpanKeys.get(writeKey) ?? []), spanKey]),
    );
  }
...
function bridgeEvidence(
  ...
  for (const bridge of bridges) {
    ...
    refs.add(
      `field-lineage:consumer-read:${consumerTaskId}:${occurrenceId}:${readRelationId}`,
    );
  }
  ...
  for (const binding of producerBindings) {
    ...
    if (strict) {
      ...
      if (
        bindingStatementId !== null &&
        bindingStatementId !== writeStatementId
      ) {
        valid = false;
    ...
    refs.add(
      `field-lineage:producer-write:${producerTaskId}:${writeObservationId}:${bindingId}`,
    );
  }
  ...
  public expand(
    ...
    if (selected.ambiguous)
      gaps.push(
        gap(
          ...
          request.source,
          [
            request.consumerLoad.evidence["field-expression-nodes.jsonl"],
    ...
    if (selected.ambiguous) {
      const candidates = new Map<
        string,
        PhysicalFieldExpansion["candidates"][number]
      >();
    ...
    const tableDecision = tableLineageDecision(
      this.context.tableLineage,
      request.consumerTaskId,
    );
    const decision = selected.classified
      ? {
          ...
          additional: selected.selected
            .filter((bridge) =>
              ["ADDITIONAL", "CANDIDATE"].includes(String(bridgeRole(bridge))),
            )
            .map((bridge) => String(bridge.producerTaskId))
            .filter(
              (taskId, index, taskIds) => taskIds.indexOf(taskId) === index,
    ...
    for (const producerTaskId of decision.additional.filter(
      (taskId) => !sameTableProducerIds.includes(taskId),
    ))
      if (
        producerMatchesField(
          this.context,
    ...
    for (const producerTaskId of decision.unknown.filter(
      (taskId) => !sameTableProducerIds.includes(taskId),
    )) {
      if (
        !producerMatchesField(
    ...
    for (const producerTaskId of decision.primary.filter(
      (taskId) => !sameTableProducerIds.includes(taskId),
    )) {
      ...
      for (const bridge of bridgeGroups) {
        ...
        if (producerPack && isSkippedLineageTask(producerPack.document))
          continue;
        const producerLoad = producerPack
          ? this.context.loadFacts(producerTaskId)
          : null;
        const writeProof =
        ...
        const producerFactsStatus = producerLoad
          ...
          : null;
        if (producerPack && producerLoad && !producerFactsStatus) {
          gaps.push(
            gap(
              request,
        ...
        if (evidenceStatus === "CONFIRMED" && producerLoad) {
          const producerFactsStatus = factsStatus(
            producerLoad,
            this.context.factsPolicy,
          );
          const consumerFactsStatus = factsStatus(
            request.consumerLoad,
            this.context.factsPolicy,
          );
          if (!producerFactsStatus || !consumerFactsStatus)
            evidenceStatus = "UNRESOLVED";
          else if (
            producerFactsStatus === "PROVISIONAL_LEGACY" ||
            consumerFactsStatus === "PROVISIONAL_LEGACY"
          )
            evidenceStatus = "PROVISIONAL_LEGACY";
        }
        ...
        if (!producerPack) {
          const excluded = taskExcludedDetail(
          ...
          producerField &&
          physicalFieldKey(producerField) !==
            physicalFieldKey(request.source) &&
          fieldMatches
        ) {
          gaps.push(
            gap(
