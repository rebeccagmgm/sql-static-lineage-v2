...
function parseCli(args: readonly string[]): CliOptions {
  ...
  const writeObservationIdsValue =
    option(args, "--write-observation-ids") ??
    option(args, "--write-observation-id") ??
    option(args, "--root-write-observation-id");
  const writeObservationIds = writeObservationIdsValue
    ?.split(",")
    .map((value) => value.trim())
    .filter(Boolean);
  const fields = (option(args, "--fields") ?? "")
    .split(",")
    .map((value) => value.trim())
    .filter(Boolean);
  const output = option(args, "--output");
  const factsPolicy = (option(args, "--facts-policy") ??
  ...
  if (
    ...
    throw new Error(
      "usage: reconcile-field-lineage --data-root <path> --facts-root <facts-root> --multi-hop-artifact <json> --task-id <id> --target-table <qualified> [--write-observation-id <id[,id...]>] [--fields <a,b>] --output <json> [--summary-output <txt>] [--timing-output <json>] [--facts-policy current-only|allow-legacy-partial]",
    );
  if (factsPolicy !== "current-only" && factsPolicy !== "allow-legacy-partial")
    throw new Error("--facts-policy is invalid");
  return {
    dataRoot,
    factsRoot,
    multiHopArtifact,
...
export function runFieldLineageCli(
  options: CliOptions,
): ReturnType<typeof reconcileFieldLineage> {
  const timings = {
  ...
  const tableLineage = JSON.parse(
    readFileSync(resolve(options.multiHopArtifact), "utf8"),
  ) as Record<string, unknown>;
  timings.table_lineage_read_ms = performance.now() - tableLineageStarted;
  const taskPathIndexStarted = performance.now();
  const taskPathIndex = options.prepareFacts
    ? indexTaskInputPacks(options.dataRoot)
    : undefined;
  ...
  const reconcile = (): ReturnType<typeof reconcileFieldLineage> =>
    reconcileFieldLineage({
  ...
  if (options.prepareFacts) {
    ...
    const prepare = (taskIds: readonly string[]): void => {
      ...
      try {
        ...
        const rootFailure = result.tasks.find(
          (task) => task.task_id === options.taskId && task.state === "FAILED",
        );
        if (rootFailure)
          throw new Error(
            rootFailure.failures.map((failure) => failure.message).join("; "),
          );
        for (const failure of result.tasks.filter(
          (task) => task.state === "FAILED",
        ))
          process.stderr.write(
            `Machine Facts preparation skipped ${failure.task_id}: ${failure.failures.map((item) => item.message).join("; ")}\n`,
          );
      } catch (error) {
        if (pending.includes(options.taskId)) throw error;
        process.stderr.write(
          `Machine Facts batch preparation failed: ${error instanceof Error ? error.message : String(error)}\n`,
        );
      } finally {
...
if (
  process.argv[1] &&
  basename(process.argv[1]).startsWith("reconcile-field-lineage")
) {
  const artifact = runFieldLineageCli(parseCli(process.argv.slice(2)));
  process.stdout.write(
    `${JSON.stringify({ output: resolve(option(process.argv.slice(2), "--output")!), status: artifact.overallStatus, counts: artifact.counts }, null, 2)}\n`,
  );
}
