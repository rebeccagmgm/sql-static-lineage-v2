...
import type { InputDependencyStatus } from "../../../machine-facts/machine-facts-contract.ts";

export const FIELD_LINEAGE_SCHEMA_VERSION = "1.2.0" as const;
export const FIELD_LINEAGE_ARTIFACT_TYPE =
  "FIELD_MULTI_HOP_RECONCILIATION" as const;

export type FieldEvidenceStatus =
  "CONFIRMED" | "PROVISIONAL_LEGACY" | "CANDIDATE" | "UNRESOLVED";
...
export type OpenLineageDirectSubtype =
...
function hashProjection(
  artifact: Omit<FieldLineageArtifact, "contentHash">,
): string {
...
export function canonicalizeFieldLineageArtifact(
  ...
  const datasetControls = sortedUnique(
    input.datasetControls,
    (item) => item.controlId,
  ).map((item) => ({
    ...item,
    evidenceRefs: [...new Set(item.evidenceRefs)].sort(compareText),
  }));
  const fieldConditionals = sortedUnique(
    input.fieldConditionals,
    (item) => item.conditionalId,
  ).map((item) => ({
    ...item,
...
const GRAINS = new Set<DatasetControlGrain>([
  "REDUCE",
  "PRESERVE",
  "EXPAND_RISK",
  "UNKNOWN",
...
export function validateFieldLineageArtifact(value: unknown): string[] {
  ...
  if (recordHas(raw, "rowsetControls"))
    errors.push("rowsetControls is not allowed");
  if (recordHas(raw, "affectedRootFields"))
    errors.push("affectedRootFields is not allowed");
  if (Array.isArray(raw.datasetControls)) {
    for (const control of raw.datasetControls) {
      ...
      if (recordHas(control, "nodeId"))
        errors.push("datasetControls must not include nodeId");
      if (recordHas(control, "affectedRootFields"))
        errors.push("affectedRootFields is not allowed");
    }
  }
  const artifact = value as FieldLineageArtifact;
  if (artifact.schemaVersion !== FIELD_LINEAGE_SCHEMA_VERSION)
    errors.push("schemaVersion is unsupported");
  if (artifact.artifactType !== FIELD_LINEAGE_ARTIFACT_TYPE)
  ...
  if (
    ...
    new Set(artifact.request.rootWriteObservationIds).size !==
      artifact.request.rootWriteObservationIds.length
  )
  ...
  for (const field of artifact.nodes?.map((node) => node.field) ?? []) {
    if (
      !field ||
      !["SCHEMA_BACKED", "TASK_LOCAL_SCHEMA_BACKED"].includes(
      ...
      errors.push(
        "all node fields must have a complete Schema-backed identity",
      );
  }
  const nodeIds = artifact.nodes?.map((node) => node.nodeId) ?? [];
  if (new Set(nodeIds).size !== nodeIds.length)
    errors.push("nodeId values must be unique");
  ...
  if (new Set(edgeIds).size !== edgeIds.length)
    errors.push("edgeId values must be unique");
  ...
  const controlIds =
    artifact.datasetControls?.map((control) => control.controlId) ?? [];
  if (new Set(controlIds).size !== controlIds.length)
    errors.push("controlId values must be unique");
  if (!ordered(controlIds))
    errors.push("datasetControls must be sorted by controlId");
  for (const control of artifact.datasetControls ?? []) {
    if (!INDIRECT_SUBTYPES.has(control.subtype))
      errors.push(
        `dataset control ${control.controlId} has an invalid subtype`,
      );
    if (!GRAINS.has(control.grain))
      errors.push(`dataset control ${control.controlId} has an invalid grain`);
    if (control.grain !== "PRESERVE") {
      if (!nonEmpty(control.grainReason))
        errors.push(
          `dataset control ${control.controlId} is missing grainReason`,
        );
      else if (!GRAIN_REASONS.has(control.grainReason))
        errors.push(
          `dataset control ${control.controlId} has an invalid grainReason`,
        );
    } else if (
      control.grainReason !== null &&
      !GRAIN_REASONS.has(control.grainReason)
    )
      errors.push(
        `dataset control ${control.controlId} has an invalid grainReason`,
      );
    if (typeof control.masking !== "boolean")
      errors.push(
        `dataset control ${control.controlId} masking must be boolean`,
      );
  }
  const conditionalIds =
    artifact.fieldConditionals?.map((item) => item.conditionalId) ?? [];
  if (new Set(conditionalIds).size !== conditionalIds.length)
    errors.push("conditionalId values must be unique");
  if (!ordered(conditionalIds))
    errors.push("fieldConditionals must be sorted by conditionalId");
  if (artifact.counts) {
    if (artifact.counts.nodes !== (artifact.nodes?.length ?? 0))
      errors.push("counts.nodes does not match");
    if (artifact.counts.edges !== (artifact.edges?.length ?? 0))
      errors.push("counts.edges does not match");
    if (
      artifact.counts.datasetControls !==
      (artifact.datasetControls?.length ?? 0)
    )
      errors.push("counts.datasetControls does not match");
    if (
      artifact.counts.fieldConditionals !==
      (artifact.fieldConditionals?.length ?? 0)
    )
      errors.push("counts.fieldConditionals does not match");
    if (artifact.counts.candidates !== (artifact.candidates?.length ?? 0))
      errors.push("counts.candidates does not match");
    if (artifact.counts.gaps !== (artifact.gaps?.length ?? 0))
      errors.push("counts.gaps does not match");
  } else errors.push("counts are required");
  if (artifact.overallStatus === "COMPLETE") {
    if ((artifact.gaps?.length ?? 0) > 0)
      errors.push("COMPLETE cannot contain gaps");
    if (artifact.limits?.truncated) errors.push("COMPLETE cannot be truncated");
    if (
      (artifact.nodes ?? []).some(
        (node) => node.evidenceStatus !== "CONFIRMED",
      ) ||
      (artifact.edges ?? []).some(
        (edge) => edge.evidenceStatus !== "CONFIRMED",
      ) ||
      (artifact.datasetControls ?? []).some(
        (control) => control.evidenceStatus !== "CONFIRMED",
      ) ||
      (artifact.fieldConditionals ?? []).some(
        (item) => item.evidenceStatus !== "CONFIRMED",
      )
    )
  ...
  if (artifact.request?.factsPolicy === "current-only") {
    if (
      ...
      (artifact.edges ?? []).some(
        (edge) => edge.evidenceStatus === "PROVISIONAL_LEGACY",
      )
    )
  ...
  if (nonEmpty(artifact.contentHash)) {
    ...
    if (hashProjection(withoutHash) !== artifact.contentHash)
