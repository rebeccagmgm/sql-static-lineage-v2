...
import type { PhysicalFieldIdentity } from "./field-lineage-contract.ts";

export type PhysicalFieldResolutionFailure =
  "TABLE_PACK_MISSING" | "TABLE_IDENTITY_AMBIGUOUS" | "FIELD_NOT_IN_SCHEMA";
...
export function resolvePhysicalInputField(
  ...
  const tailMatches =
    rawTable.includes(".") || context.defaultSchema !== null
      ...
      : context.catalog.entries.filter(
          (entry) =>
            normalizeName(entry.qualifiedName).split(".").at(-1) === rawTable,
        );
  const tables =
  ...
  const local = taskLocalResolution(context, rawTable, qualifiedTable, column);
