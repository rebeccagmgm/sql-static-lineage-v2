...
function manifestSqlSource(
  ...
  const inputs =
    typeof manifest.inputs === "object" && manifest.inputs !== null
...
function rootCriterion(
  input: Omit<RootCriterion, "rootCriterionId">,
): RootCriterion {
  return {
    ...input,
...
export function resolveWriteScopedPlanInputs(
  ...
  if (!sqlSource) {
    addGap(
      makeGap({
        ...
        message:
          "Machine Facts manifest does not identify one immutable SQL snapshot",
  ...
  for (const writeObservationId of requestedWrites) {
    ...
    if (!canonical.occurrence) {
      if (canonical.reasonCode === "OUTPUT_BINDING_MISSING") {
        const fields =
          requestedTargetFields.length > 0 ? requestedTargetFields : [null];
      ...
      if (canonical.reasonCode === "ROOT_RELATION_UNMAPPED") {
        ...
        const fields =
          requestedTargetFields.length > 0
            ? requestedTargetFields
            : unique(
                rawWriteBindings
                  .map((row) => normalized(row.target_field))
                  .filter(Boolean),
              );
        const conflictingFields = fields.filter(
          (field) =>
            rawWriteBindings.filter(
              (row) => normalized(row.target_field) === field,
            ).length > 1,
        );
    ...
    for (const targetFieldName of targetFields) {
      const bindingMatch = exactRecord(
        writeBindings,
        (row) => normalized(row.target_field) === targetFieldName,
      );
      if (!bindingMatch.row) {
        addGap(
          makeGap({
      ...
      const expressionMatch = exactRecord(
        expressions,
        (row) =>
          text(row.task_id) === input.taskId &&
          text(row.expression_id) === expressionId,
      );
      ...
      const relationMatch = exactRecord(
        relations,
        (row) =>
          text(row.task_id) === input.taskId &&
          text(row.relation_id) === relationId,
      );
      ...
      if (
        ...
        !localOutputExpressionId.startsWith(
          `${localRootRelationId}:expression:`,
        )
      ) {
        addGap(
          makeGap({
      ...
      try {
        rootTargetFieldId = text(
          input.resolveRootTargetFieldId(targetFieldName, binding),
        );
      } catch {
  ...
  return {
    rootCriteria: criteria.sort((left, right) =>
