...
export interface TargetTableCausalClosureArtifact {
  ...
  readonly relationSummaries: readonly {
    readonly taskId: string;
...
export function relationRank(status: RelationStatus): number {
  return status === "CONFIRMED_RELATED"
    ? 3
...
export function createNegativeProof(
  input: Omit<NegativeProof, "proofId">,
): NegativeProof {
  const normalized = {
    ...input,
    closedChannels: [...input.closedChannels]
      .sort((left, right) => left.channel.localeCompare(right.channel))
      .map((channel) => ({ ...channel, proofRefs: sorted(channel.proofRefs) })),
    premiseRefs: sorted(input.premiseRefs),
  };
  return {
    ...normalized,
    proofId: `negative-proof:${sha256(canonicalJson(normalized))}`,
  };
}
...
export function canonicalizeTargetTableArtifact(
  ...
  const assessments = [...input.assessments].sort((left, right) =>
  ...
  const stable = {
    ...
    ...(input.shrinkReport
      ? {
          shrinkReport: {
            ...
            rowDetermining: sortShrinkEntries(
              input.shrinkReport.rowDetermining,
            ),
            multiplicityRisk: sortShrinkEntries(
              input.shrinkReport.multiplicityRisk,
            ),
            prunedCount: input.shrinkReport.prunedCount,
            prunedReasons: [...input.shrinkReport.prunedReasons]
              .sort((left, right) =>
    ...
    relationSummaries: [...input.relationSummaries].sort(
      (left, right) =>
        left.taskId.localeCompare(right.taskId) ||
        left.sqlSourceId.localeCompare(right.sqlSourceId) ||
...
export function candidateBranchFor(
  ...
  return universe.branches.find(
    (branch) => branch.candidateBranchId === branchId,
  );
}
