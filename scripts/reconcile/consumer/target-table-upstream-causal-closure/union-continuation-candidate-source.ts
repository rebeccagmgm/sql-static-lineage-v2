...
export type ContinuationPartitionMatchStatus =
  "CONFIRMED" | "ASSUMED" | "UNKNOWN" | "DISJOINT";
export type ContinuationEvidenceLayer = "L1" | "L2";
export type ContinuationPartitionPredicateStatus =
...
const PARTITION_PREDICATE_STATUSES =
  new Set<ContinuationPartitionPredicateStatus>([
    "NONE",
    "LITERAL",
    "NON_LITERAL_PRESENT",
  ]);
...
function parseTaskProjection(
  value: unknown,
  path: string,
): UnionContinuationIndexTaskProjectionRef {
...
function parseCandidate(
  ...
  return {
    taskId: text(source.taskId, `${path}.taskId`),
    writeObservationId: text(
      source.writeObservationId,
      `${path}.writeObservationId`,
    ),
    targetWriteNodeId: nullableText(
      source.targetWriteNodeId,
      `${path}.targetWriteNodeId`,
    ),
    ...
    partitionMatchStatus: enumValue(
      source.partitionMatchStatus,
      PARTITION_MATCH_STATUSES,
      `${path}.partitionMatchStatus`,
    ),
    partition: recordArray(source.partition, `${path}.partition`),
    evidenceLayer: enumValue(
      source.evidenceLayer,
      EVIDENCE_LAYERS,
      `${path}.evidenceLayer`,
    ),
    l1Eligible: boolean(source.l1Eligible, `${path}.l1Eligible`),
    ...(source.alignmentGapCode === undefined
      ? {}
      : {
          alignmentGapCode: text(
            source.alignmentGapCode,
            `${path}.alignmentGapCode`,
          ),
        }),
...
function parseEntry(value: unknown, path: string): UnionContinuationIndexEntry {
  const source = record(value, path);
  const candidates = array(source.candidates, `${path}.candidates`).map(
    (item, index) => parseCandidate(item, `${path}.candidates[${index}]`),
  );
  const candidateKeys = new Set<string>();
  for (const candidate of candidates) {
    const key = `${candidate.taskId}\u0000${candidate.writeObservationId}`;
    if (candidateKeys.has(key))
      throw new Error(`UNION_CONTINUATION_INDEX_CANDIDATE_DUPLICATE:${key}`);
  ...
  const entry: UnionContinuationIndexEntry = {
    ...
    readOccurrenceNodeId: text(
      source.readOccurrenceNodeId,
      `${path}.readOccurrenceNodeId`,
    ),
    datasetNodeId: text(source.datasetNodeId, `${path}.datasetNodeId`),
    qualifiedName: text(source.qualifiedName, `${path}.qualifiedName`),
    identityStatus: text(source.identityStatus, `${path}.identityStatus`),
    partitionPredicateStatus: enumValue(
      source.partitionPredicateStatus,
      PARTITION_PREDICATE_STATUSES,
      `${path}.partitionPredicateStatus`,
    ),
    candidates,
    prunedWriteObservationIds: stringArray(
      source.prunedWriteObservationIds,
      `${path}.prunedWriteObservationIds`,
    ),
    gaps: array(source.gaps, `${path}.gaps`).map((item, index) =>
      parseGap(item, `${path}.gaps[${index}]`),
    ),
  };
  for (const candidate of entry.candidates) {
    if (
      ...
      throw new Error(
        `UNION_CONTINUATION_INDEX_PRUNED_CANDIDATE_MISSING:${candidate.writeObservationId}`,
      );
    }
    if (
      ...
      throw new Error(
        `UNION_CONTINUATION_INDEX_L1_ELIGIBILITY_INVALID:${candidate.writeObservationId}`,
      );
    }
  }
  return entry;
...
function parseIndex(value: unknown): UnionContinuationIndex {
  ...
  const projectionValues = array(
    inputSource.taskProjections,
    "input.taskProjections",
  );
  const taskProjections = projectionValues.map((item, index) =>
    parseTaskProjection(item, `input.taskProjections[${index}]`),
  );
  if (
    taskProjections.some(
      (projection) =>
        !taskLocalSchemaVersionAtLeast(
          projection.schemaVersion as "1.1.0" | "1.2.0" | "1.3.0",
          "1.2.0",
        ),
    )
  ) {
  ...
  for (const projection of taskProjections) {
    if (taskIds.has(projection.taskId))
      throw new Error(
        `UNION_CONTINUATION_INDEX_INPUT_TASK_DUPLICATE:${projection.taskId}`,
      );
    taskIds.add(projection.taskId);
  }
  const entries = array(source.entries, "entries").map((item, index) =>
    parseEntry(item, `entries[${index}]`),
  );
  ...
  const input: UnionContinuationIndexInput = {
    batchManifestRef: record(
      inputSource.batchManifestRef,
      "input.batchManifestRef",
    ),
...
export function loadUnionContinuationCandidateSource(
  path: string,
  ...
  try {
    ...
    throw new Error(
      `UNION_CONTINUATION_INDEX_READ_FAILED:${path}:${error instanceof Error ? error.message : String(error)}`,
    );
  }
  return createUnionContinuationCandidateSource(parsed);
}

export function createUnionContinuationCandidateSource(
  value: unknown,
): UnionContinuationCandidateSource {
  ...
  return {
    ...
    candidatesForRead: (consumerTaskId, readOccurrenceId) =>
      entries.get(`${consumerTaskId}\u0000${readOccurrenceId}`)?.candidates ??
      [],
    entryForRead: (consumerTaskId, readOccurrenceId) =>
      entries.get(`${consumerTaskId}\u0000${readOccurrenceId}`),
  };
}
...
export function continuationIndexEntryReference(
  ...
  return indexEntryRef(source.index.contentHash, {
    consumerTaskId,
    readOccurrenceId,
  });
}
