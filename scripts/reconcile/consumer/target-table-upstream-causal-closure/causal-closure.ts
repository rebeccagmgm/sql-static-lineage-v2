...
function mergeState(
  ...
  const preferred =
    certaintyRank(left.certainty) >= certaintyRank(right.certainty)
      ? left
      : right;
  return {
    ...preferred,
    ...
    localTransferKinds: unique([
      ...left.localTransferKinds,
      ...right.localTransferKinds,
    ]) as LocalTransferKind[],
    demandedFieldNames: unique([
      ...left.demandedFieldNames,
      ...right.demandedFieldNames,
    ]),
    outputFieldBindingIds: unique([
      ...(left.outputFieldBindingIds ?? []),
      ...(right.outputFieldBindingIds ?? []),
    ]),
    affectedTargetFields: unique([
      ...(left.affectedTargetFields ?? []),
      ...(right.affectedTargetFields ?? []),
    ]),
  };
}
...
function demandFieldsForChannel(
...
function branchBelongsToWriteScope(
  ...
  const source = canonicalSqlSourceId(
    occurrence.sqlSourceId ?? occurrence.occurrenceId,
  );
  if (
    !sameText(source, scope.sqlSourceId) ||
    occurrence.statementIndex !== scope.statementOrdinal
  )
    return false;
  if (
    occurrence.rootRelationId &&
    !sameText(occurrence.rootRelationId, scope.rootRelationId)
  )
    return false;
  ...
  if (
    ...
    sameText(summary.rootRelationId, scope.rootRelationId)
  )
    return true;
  return occurrence.relationPath.some((relationId) =>
    sameText(relationId, scope.rootRelationId),
  );
}
...
function scopeForBranch(branch: CandidateBranch): WriteScope | null {
  ...
  if (
    branch.gapRefs.some((gap) =>
      /CONTINUATION_(?:READ_NOT_FOUND|PRODUCER_NOT_FOUND)/i.test(gap),
    )
  )
    return null;
  if (
    !branch.continuation &&
    branch.gapRefs.some((gap) =>
      /PRODUCER_WRITE|WRITE_OBSERVATION|WRITE_SCOPE/i.test(gap),
    )
  )
    return null;
  return {
...
function channelMap(
  values: readonly ChannelAssessment[],
): ReadonlyMap<ImpactChannel, ChannelAssessment> {
  return new Map(values.map((value) => [value.channel, value]));
}

function propagatedChannelAssessment(
...
function continuationCertainty(branch: CandidateBranch): PathCertainty | null {
  ...
  if (
    continuation.source === "IN_UNION_FINAL_WRITE" &&
    continuation.partitionMatchStatus === "ASSUMED"
  )
    return "CONDITIONAL";
  return "UNKNOWN";
}

function finalRelationStatus(
  channels: readonly ChannelAssessment[],
): "CONFIRMED_RELATED" | "CONDITIONAL_RELATED" | "UNKNOWN" {
  if (channels.some((channel) => channel.status === "CONFIRMED"))
    return "CONFIRMED_RELATED";
  if (channels.some((channel) => channel.status === "CONDITIONAL"))
    return "CONDITIONAL_RELATED";
  return "UNKNOWN";
}
...
function finalAssessment(input: {
  ...
  for (const [channel, state] of input.propagated)
    channels.set(channel, propagatedChannelAssessment(state));
  const localFieldValue = input.local.find(
    (channel) => channel.channel === "FIELD_VALUE",
  );
  for (const local of input.local) {
    const prior = channels.get(local.channel);
    const leftDimMultiplicity =
      localFieldValue?.status === "NOT_APPLICABLE" &&
    ...
    channels.set(local.channel, {
      ...
      gapRefs: unique([
        ...local.gapRefs,
        gap,
        ...(input.propagationGaps ?? []),
      ]),
    });
  }
  if (
    channels.size === 0 ||
    [...channels.values()].every(
      (channel) => channel.status === "NOT_APPLICABLE",
    )
  ) {
  ...
  const evidenceRefs = unique([
    ...
    ...channelAssessments.flatMap((channel) => [
      ...channel.proofRefs,
      ...channel.witnessRefs,
    ]),
  ]);
...
interface PropagationRun {
  readonly statesByBranch: ReadonlyMap<
    string,
    ReadonlyMap<ImpactChannel, PropagationState>
  >;
  readonly reachableTaskIds: readonly string[];
  readonly reachableBranchIds: readonly string[];
...
function propagate(input: {
  ...
  const root = input.universe.branches.find(
    (branch) => branch.branchKind === "ROOT_WRITE",
  );
  const rootWriteObservationId =
    root?.writeObservationId ?? `target-write:${input.targetWriteId}`;
  ...
  const seedCertainty: PathCertainty =
    root && root.gapRefs.length > 0 ? "UNKNOWN" : "CONFIRMED";
  ...
  const statesByBranch = new Map<
    string,
    Map<ImpactChannel, PropagationState>
  >();
  ...
  for (const branch of input.universe.branches) {
    ...
    const source = canonicalSqlSourceId(
      branch.readOccurrence.sqlSourceId ?? branch.readOccurrence.occurrenceId,
    );
  ...
  const deadlineReached = (): boolean =>
    input.budget?.deadlineAt !== undefined &&
    performance.now() >= input.budget.deadlineAt;
  ...
  const mergeBranchState = (
    branch: CandidateBranch,
    next: PropagationState,
  ): void => {
    const branchStates =
      statesByBranch.get(branch.candidateBranchId) ??
      new Map<ImpactChannel, PropagationState>();
  ...
  while (pending.length > 0 && !stopped) {
    ...
    for (const upstream of input.sameTaskUpstreamWrites?.get(
      `${currentScope.taskId}|${currentScope.writeObservationId}`,
    ) ?? []) {
      ...
      const upStates =
        nodeStates.get(upKey) ?? new Map<ImpactChannel, PropagationState>();
      let changed = false;
    ...
    for (const branch of branchesByReadScope.get(readScopeKey) ?? []) {
      ...
      if (!branchBelongsToWriteScope(branch, currentScope, branchSummary))
        continue;
      const matchedScopes =
        matchedConsumerScopes.get(branch.candidateBranchId) ??
      ...
      const local = channelMap(
        localByBranch.get(branch.candidateBranchId) ?? [],
      );
      const emit = (
        ...
        const downstream =
          downstreamOverride ?? currentStates.get(targetChannel);
        ...
        const common = {
          ...
          writeObservationId:
            producerScope?.writeObservationId ??
            `unresolved:${branch.candidateBranchId}`,
          sqlSourceId: producerScope?.sqlSourceId ?? currentScope.sqlSourceId,
          statementOrdinal:
            producerScope?.statementOrdinal ?? currentScope.statementOrdinal,
          rootRelationId:
            producerScope?.rootRelationId ?? currentScope.rootRelationId,
        ...
        const next: PropagationState = {
          ...
          gapRefs: unique([
            ...
            ...(unresolvedWrite
              ...
              : []),
            ...(depthExceeded
              ? [`causal-closure-gap:${branch.candidateBranchId}:WITNESS_DEPTH`]
              : []),
          ]),
        };
        if (!stateUpdateAllowed()) {
          const gap = budgetGap(
            deadlineReached() ? "DEADLINE" : "MAX_STATE_UPDATES",
          );
          mergeBranchState(branch, {
            ...next,
            certainty: "UNKNOWN",
            gapRefs: unique([...next.gapRefs, gap]),
          });
          return;
        }
        ...
        if (!existingProducerStates && nodeStates.size >= maxNodeStates) {
          mergeBranchState(branch, {
            ...next,
            certainty: "UNKNOWN",
            gapRefs: unique([...next.gapRefs, budgetGap("MAX_NODE_STATES")]),
          });
          return;
        }
        mergeBranchState(branch, next);
        ...
        const producerStates =
          existingProducerStates ?? new Map<ImpactChannel, PropagationState>();
      ...
      const recalledAsValue =
        valueCarrier !== undefined && currentScope.taskId !== input.rootTaskId;
      const localControl = local.get("EXPRESSION_CONTROL");
      const zipperControl =
        localControl !== undefined && localControl.status !== "NOT_APPLICABLE";
      for (const channel of CAUSAL_IMPACT_CHANNELS) {
        const localValue = local.get(channel);
        if (!localValue || localValue.status === "NOT_APPLICABLE") continue;
        const demanded =
          localValue.demandedFieldNames ??
          currentStates.get(channel)?.demandedFieldNames ??
          [];
        ...
        if (valueCarrier && ROW_CONTROL_CHANNELS.has(channel)) {
          if (channel === "MULTIPLICITY" || channel === "RELATION_EXISTENCE")
            continue;
      ...
      if (fieldValue && fieldValue.status !== "NOT_APPLICABLE") {
        for (const targetChannel of [
          ...
          const matched = outputs.filter((output) =>
            demanded.some((field) => sameText(output, field)),
          );
          if (!downstream || matched.length === 0) continue;
          if (
            targetChannel === "ROW_MEMBERSHIP" &&
            recalledAsValue &&
            !zipperControl
          )
            continue;
  ...
  for (const [branchId, branchStates] of statesByBranch) {
    const branch = input.universe.branches.find(
      (candidate) => candidate.candidateBranchId === branchId,
    );
    if (!branch) continue;
    const matchedScopes = matchedConsumerScopes.get(branchId);
    if (
      !matchedScopes ||
      [...matchedScopes.values()].some(
        (scope) =>
          !branchBelongsToWriteScope(
            branch,
            scope,
            summaryForBranch(input.summaries, branch),
          ),
      )
    ) {
...
export function buildCausalClosure(input: {
  ...
  const base =
    input.baseGraph ??
    buildImpactGraph(input.universe.branches, input.summaries);
    ...
    branchEdges: input.universe.branches
      .filter(
        (branch) =>
          ...
          propagated.reachableBranchIds.includes(branch.candidateBranchId) &&
          branch.consumerTaskId !== null,
      )
      .map((branch) => ({
        branchId: branch.candidateBranchId,
  ...
  const assessments = input.universe.branches.map((branch) => {
    ...
    const states =
      propagated.statesByBranch.get(branch.candidateBranchId) ??
      new Map<ImpactChannel, PropagationState>();
  ...
  const rollup = rollupAssessments({
    branches: input.universe.branches,
    assessments,
  });
  const gaps = unique([
    ...propagated.gaps,
    ...assessments.flatMap((assessment) => assessment.gapRefs),
  ]).map((gapId) => ({
    gapId,
    reasonCode: gapId.includes("causal-closure-gap")
      ? "CAUSAL_CLOSURE_BOUNDARY"
      : "CAUSAL_EVIDENCE_INCOMPLETE",
    message: `causal closure could not close ${gapId}`,
    evidenceRefs:
      assessments.find((assessment) => assessment.gapRefs.includes(gapId))
        ?.evidenceRefs ?? [],
  }));
  return {
    graph,
